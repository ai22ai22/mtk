...
ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN static uint8_t rd_buffer[SPI_TEST_DATA_SIZE + 1];
...
static void spi_master_send_data_two_boards_example(void)
{
    ...
    if (HAL_SPI_MASTER_STATUS_OK != hal_spi_master_register_callback(SPI_TEST_MASTER, spi_master_dma_callback, NULL)) {
        log_hal_error("SPI master register callback failed\r\n");
        return;
    }
    ...
    for (i = 1; i < (SPI_TEST_DATA_SIZE + 1); i++) {
        wr_buffer[i] = (i % SPI_TEST_DATA_RANGE);
    }
    ...
    /* wait SPI slave to finish data check */
    hal_gpt_delay_ms(500);

    /* Step6: Master starts to send cfg_rd_cmd command to slave */
    ...
    if (HAL_SPI_MASTER_STATUS_OK != hal_spi_master_send_polling(SPI_TEST_MASTER, cfg_rd_cmd, 9)) {
        log_hal_error("SPI master send cfg_rd_cmd command failed\r\n");
        return;
    }
    do {
        spi_query_slave_status(&status_receive);
    } while ((status_receive & (uint8_t)SPISLV_STATUS_TXRX_FIFO_RDY_MASK) != (uint8_t)SPISLV_STATUS_TXRX_FIFO_RDY_MASK);
    printf("SPI master sends cfg_rd_cmd command successfully\r\n\r\n");

    /* Step7: Master starts to send rd_cmd command to slave */
    printf("SPI master starts to send rd_cmd command to slave using DMA mode\r\n");
    printf("Size to receive = %d\r\n", SPI_TEST_DATA_SIZE);
    g_spi_transaction_finish = false;
    wr_buffer[0] = SPIS_RD_CMD;
    memset(rd_buffer, 0x00, sizeof(rd_buffer));
    send_and_receive_config.send_data = wr_buffer;
    send_and_receive_config.send_length = 1;
    send_and_receive_config.receive_buffer = rd_buffer;
    send_and_receive_config.receive_length = SPI_TEST_DATA_SIZE + send_and_receive_config.send_length;
    if (HAL_SPI_MASTER_STATUS_OK != hal_spi_master_send_and_receive_dma(SPI_TEST_MASTER, &send_and_receive_config)) {
        log_hal_error("\r\n SPI master send rd_cmd command failed\r\n");
    }
    while (g_spi_transaction_finish == false);
    do {
        spi_query_slave_status(&status_receive);
    } while ((status_receive & (uint8_t)SPISLV_STATUS_RDWR_FINISH_MASK) != (uint8_t)SPISLV_STATUS_RDWR_FINISH_MASK);
    printf("SPI master sends rd_cmd command successfully\r\n\r\n");

    /* Step8: Master begin to check the data received from slave */
    printf("Start to check data integrity\r\n");
    for (i = 1; i < (SPI_TEST_DATA_SIZE + 1); i++) {
        if (rd_buffer[i] != (i % SPI_TEST_DATA_RANGE)) {
    ...
    /* Step9: Master starts to send power_off command to slave and query whether it's successful. */
    ...
    /* Step10: De-initialize SPI master and GPIO after data transaction end. */
