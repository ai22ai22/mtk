...
#define AUDIO_PLAYER_FLAG_STOPPING          (1 << 3)  // indicate in stopping music
#define AUDIO_PLAYER_FLAG_CHANGE_TRACK      (1 << 4)  // indicate change track
#define AUDIO_PLAYER_FLAG_MP3_AUDIO_SWITCH  (1 << 5)  // indicate switch audio source when receive aws suspend cnf
#define AUDIO_PLAYER_FLAG_MP3_AUDIO_SOURCE  (1 << 6)  // indicate it's audio source device
#define AUDIO_PLAYER_FLAG_SNK_INTERRUPT     (1 << 7)
#define AUDIO_PLAYER_FLAG_SNK_NEED_RESYNC   (1 << 8)  // indicate play comes after init sync in sink, so need resync
#define AUDIO_PLAYER_FLAG_PLAYING           (1 << 9)  // indicate it's after initial sync
#define AUDIO_PLAYER_FLAG_RESYNCING         (1 << 10) // indicate it's resyncing
...
#define AUDIO_PLAYER_RING_BUFFER_SIZE       (30 * 1024) /* audio ring buffer size */
#define AUDIO_PLAYER_FILE_READ_MAX_SIZE     (1 * 1024) /* max read file buffer size */
#define AUDIO_PLAYER_FILE_READ_COUNT        (3) /* read file count each task schedule */
#define AUDIO_PLAYER_BUFFER_THRESHOLD       (4 * 1024) /* start playing threshold */
...
#define BT_SINK_SRV_MP3_AWS_TIMER_DUR            (150 * 1000)  /* ums*/
#define AUDIO_PLAYER_AWS_RESYNC_THRESHOLD        (10) /* suspend music if reached resync attempt */
...
typedef struct {
    ...
    hal_gpt_port_t gpt_port;      /** For aws + mp3 */
