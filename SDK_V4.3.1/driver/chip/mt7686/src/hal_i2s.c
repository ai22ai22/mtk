...
hal_i2s_status_t hal_i2s_set_config_ex(hal_i2s_port_t i2s_port, const hal_i2s_config_t *config)
{
    ...
    switch (config->i2s_out.word_select_inverse) {
        case HAL_I2S_WORD_SELECT_INVERSE_EBABLE:
            i2s_internal_cfg[i2s_port].i2s_user_config.i2s_out.word_select_inverse = HAL_I2S_WORD_SELECT_INVERSE_EBABLE;
            break;
        default:
    ...
    switch (config->i2s_in.word_select_inverse) {
        case HAL_I2S_WORD_SELECT_INVERSE_EBABLE:
            i2s_internal_cfg[i2s_port].i2s_user_config.i2s_in.word_select_inverse = HAL_I2S_WORD_SELECT_INVERSE_EBABLE;
            break;
        default:
    ...
    if (i2s_port == HAL_I2S_0) {
        if ((i2s_internal_cfg[i2s_port].i2s_initial_type == HAL_I2S_TYPE_EXTERNAL_TDM_MODE) || (i2s_internal_cfg[i2s_port].i2s_initial_type == HAL_I2S_TYPE_INTERNAL_TDM_LOOPBACK_MODE)) {
            ...
            if ((config->frame_sync_width == HAL_I2S_FRAME_SYNC_WIDTH_128) && (config->clock_mode == HAL_I2S_MASTER)) {
                log_hal_error("I2S%d] CONFIG ERROR, invalid frame_sync_width, HAL_I2S_FRAME_SYNC_WIDTH_128 is not supported on TDM master mode\r\n");
                return HAL_I2S_STATUS_INVALID_PARAMETER;
    ...
    if ((i2s_internal_cfg[HAL_I2S_0].i2s_state == I2S_STATE_INIT) && (i2s_internal_cfg[HAL_I2S_1].i2s_state == I2S_STATE_INIT)) {
...
hal_i2s_status_t hal_i2s_stop_tx_vfifo_ex(hal_i2s_port_t i2s_port)
{
    ...
    //workaround for vdma bug
    //status = vdma_stop(i2s_vdma_rx_ch[i2s_port]);
    //if (status != VDMA_OK) {
    //    log_hal_error("[I2S%d] STOP TX VFIFO ERROR, workaround fail\r\n", i2s_port);
    //    return HAL_I2S_STATUS_ERROR;
    //}
...
{
    /*
        vdma_status_t status = VDMA_OK;
        status = vdma_push_data_4bytes(i2s0_pdma_tx_channel, data);
        if (status != VDMA_OK) {
            log_hal_error("[i2s0] vdma_push_data_4bytes fail\r\n");
            return HAL_I2S_STATUS_ERROR;
        }
    */
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
        if (is_lock_sleep[HAL_I2S_0] == false) {
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
        if (is_lock_sleep[HAL_I2S_1] == false) {
...
hal_i2s_status_t  hal_i2s_disable_tx_ex(hal_i2s_port_t i2s_port)
{
    ...
    if (i2s_port == HAL_I2S_0) {
        ...
        if (is_lock_sleep[i2s_port] == true) {
            if ((I2S_CHECK_BIT(i2s_internal_cfg[i2s_port].i2s_state, I2S_STATE_TX_RUNNING) == 0) && (I2S_CHECK_BIT(i2s_internal_cfg[i2s_port].i2s_state, I2S_STATE_RX_RUNNING) == 0)) {
...
{
    log_hal_info("[I2S%d] ADUIOTOP ENABLE\r\n", i2s_port);
    ...
    if (i2s_port == HAL_I2S_0) {
        ...
        //printf("I2S1_DL_SR_EN_CONTROL=%x\r\n", (unsigned int)DRV_Reg32(I2S1_DL_SR_EN_CONTROL));
        //printf("I2S1_GLOBAL_EN_CONTROL=%x\r\n", (unsigned int)DRV_Reg32(I2S1_GLOBAL_EN_CONTROL));
        //printf("I2S1_GLOBAL_EN_CONTROL__F_CR_PDN_AUD_26M=%x\r\n", (unsigned int)DRV_Reg8(I2S1_GLOBAL_EN_CONTROL__F_CR_PDN_AUD_26M));
    }
...
{
    log_hal_info("[I2S%d] ADUIOTOP DISABLE\r\n", i2s_port);
...
{
    log_hal_info("[I2S%d] TX DMA INTERRUPT ENABLE\r\n", i2s_port);
...
{
    log_hal_info("[I2S%d] TX DMA INTERRUPT DISABLE\r\n", i2s_port);
...
{
    log_hal_info("[I2S%d] RX DMA INTERRUPT ENABLE\r\n", i2s_port);
...
{
    log_hal_info("[I2S%d] RX DMA INTERRUPT DISABLE\r\n", i2s_port);
...
void i2s0_enter_suspend(void)
{
    ...
    //hal_gpio_init(HAL_I2S0_MCLK_PIN);
    //hal_pinmux_set_function(HAL_I2S0_MCLK_PIN,5);
...
{
    log_hal_info("[I2S0] ENTER RESUME\r\n");
    hal_i2s_port_t i2s_port = HAL_I2S_0;

    if ((i2s_internal_cfg[i2s_port].i2s_state == I2S_STATE_INIT) && (i2s_internal_cfg[i2s_port].i2s_configured)) {
        /*reinit and reconfig when i2s resume back*/
        hal_i2s_set_config_ex(i2s_port, &i2s_internal_cfg[i2s_port].i2s_user_config);

        /*reconfig vfifo*/
        if (i2s_internal_cfg[i2s_port].i2s_vfifo.tx_dma_configured) {
            hal_i2s_setup_tx_vfifo_ex(
                i2s_port,
        ...
        //we do not need to enable i2s and clock now, because i2s can not be allowed to enter sleep when i2s tx or rx is enabled
...
{
    log_hal_info("[I2S1] ENTER RESUME\r\n");

    hal_i2s_port_t i2s_port = HAL_I2S_1;
