...
{
    //INFO(connsys, "[EINT]Wifi_eint_handler\n");
}
#endif
void register_wifi_intr_handler()
{
    ...
    config_set.trigger_mode = HAL_EINT_LEVEL_LOW;
...
{
    // Don't Remove! It's used to wakeup CM4 tickless mode.
    INFO(connsys, "[EINT] connsys_eint_handler\n");
}
#endif
...
int32_t connsys_open()
{
...
#if (PRODUCT_VERSION == 7686 || PRODUCT_VERSION == 7682 || PRODUCT_VERSION == 5932)
	    hal_gdma_init(CONNSYS_GDMA_CH);
	    hal_gdma_register_callback(CONNSYS_GDMA_CH, connsys_dma_callback, NULL);
	    // Enable GDMA
    ...
    // EINT unmask by SPM module if CM4 enter tickless mode
...
static void _connsys_init_activate_mcu(void)
{
    ...
    //Wait ROM Initialize done
    while (HAL_REG_32(N9ROM_INIT_DONE) != 1)
        ;
}
...
int32_t connsys_init(sys_cfg_t *sys_config)
{
    static ATTR_ZIDATA_IN_TCM connsys_state_t   _g_connsys_state = CONNSYS_STATE_UNINITIALIZED;
