...
//pcm playback use
static uint8_t *audio_internal_ring_buffer = 0;
//mp3 decoder lib use
...
static int32_t previous_mp3_frame_size = SHARE_BUFFER_TOO_LESS_AMOUNT;
    ...
    if (IS_MP3_HEAD(temp_mp3_header)) {
        ...
        *mp3_frame_size = frameSize;
        if (IS_MP3_HEAD(temp_mp3_header)) {
            ...
            log_hal_info("[MP3 Codec] The next frame header is invalid: 0x%04X\n", temp_mp3_header);
...
static void mp3_codec_deocde_hisr_handler(void *data)
{
    ...
    if (handle->flush_data_flag == 1) {
        ...
        mp3_frame_size = (mp3_frame_size > 0)?mp3_frame_size:previous_mp3_frame_size;
        previous_mp3_frame_size = mp3_frame_size;
        check_min_share_size = mp3_frame_size;
    }

    if (share_buffer_data_amount < check_min_share_size) {
    ...
    if ((consumeBS != mp3_frame_size) && (handle->flush_data_flag != 1)) {
...
static mp3_codec_function_return_state_t mp3_codec_play_internal(mp3_codec_media_handle_t *handle)
{
    ...
    audio_internal_ring_buffer = (uint8_t *)pvPortMalloc(memory_size * sizeof(uint8_t)); // 8 byte alignment
    log_hal_info("[MP3 Codec] pvPortMalloc(%d) -, audio_internal_ring_buffer = 0x%X\n", memory_size * sizeof(uint8_t), audio_internal_ring_buffer);
