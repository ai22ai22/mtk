...
static void amr_encoder_hisr_handler(void *data)
{
    ...
    while ((ring_buffer_get_space_byte_count(&internal_handle->encoded_pcm_buff)) >= 32) { //32bytes is the possible maximum size currently
...
#if 0
    //LOGI(" data in encoded_pcm_buff=%d bytes\r\n",ring_buffer_get_data_byte_count(&internal_handle->encoded_pcm_buff));
    if (internal_handle->frame >= 10) {
        if (!handle->waiting) {
            handle->waiting = true;
            handle->handler(handle, AMR_ENCODER_MEDIA_EVENT_DATA_NOTIFICATION);
        }
