...
void wfcm_sdio_fw_download(void)
{
  ...
  if ((get_bin->m_magic == 0x444D434D) && (get_bin->m_msg_id == GET_BIN)) {
    ...
    cmd->m_reserve[1] = MXA_BLOCK_SIZE;
  ...
  // Send binary
  {
    ...
    for (; total_length>0; ) {
      memcpy(brom_tx_test, pBuf8, MXA_BLOCK_SIZE);
      pBuf8 += MXA_BLOCK_SIZE;
      
      if (total_length >= MXA_BLOCK_SIZE)
      {
        len_segment = MXA_BLOCK_SIZE;
      } else {
