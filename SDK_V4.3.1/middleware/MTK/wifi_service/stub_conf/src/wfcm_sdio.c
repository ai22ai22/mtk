...
bool wfcm_sdio_write(uint32_t  port_offset, uint8_t *write_buffer, uint16_t length, uint8_t usingDMA)
{
    ...
    if (usingDMA)
        status = hal_sdio_execute_command53_dma(WIFI_SDIO_PORT, &cmd_cfg);
    else
...
void wfcm_sdio_cmd_cp(uint16_t opcode, T_WFC_QBUF *qbuf, uint8_t *param, uint32_t paramSize)
{
    uint16_t *op;
    uint8_t  *pdst;
...
bool wfcm_sdio_send_data(T_WFC_QBUF *qbuf, uint8_t qbuf_release)
{
    ...
    //Free HTX Buffer
    if (qbuf_release)  wfcm_mq_free(qbuf);
...
bool wfcm_sdio_receive_data(T_WFC_QBUF *qbuf)
{
	uint32_t whisr_reg = 0;
	uint32_t wrplr_reg = 0;	
    ...
    wfcm_sdio_read(SDIO_IP_WRPLR, &wrplr_reg, 4, false);
    if (wfcm_sdio_read(SDIO_IP_WRDR0, (uint32_t*)&(g_host_rx_buf[qbuf->qidx][0]), (wrplr_reg&0xffff), false) == false) {        
        printf("[ERR],sdio_receive_pkt, wfcm_sdio_read SDIO_IP_WRDR0 fail\r\n");
    	return false;
    }
