...
#define WFC_STUB_TASK_PRIORITY       6    


#define WFC_QUEUE_TASK_NAME       "wfq"
#define WFC_QUEUE_TASK_STACK_SIZE  512
#define WFC_QUEUE_TASK_PRIORITY      6
#define WFC_QUEUE_SIZE              10

#define WFC_MQ_TASK_NAME          "wfm"
#define WFC_MQ_TASK_STACK_SIZE     512
#define WFC_MQ_TASK_PRIORITY         6
...
struct pbuf *wfc_sim_pbuf_alloc(uint32_t pbufSize)
{
    ...
    #if 0 //pass lwip
    return (uint8_t *)malloc(pbufSize);
    #else
...
uint8_t wfc_send_to_host(uint8_t *pbuf, uint32_t bufSize)
{
    ...
    qbuf = (T_WFC_QBUF *)malloc(sizeof(T_WFC_QBUF));
...
void wfc_stub_handler(T_WFC_OPCODE *pwfc)
{
    ...
    if ((pwfc->opcmd != WFC_CMD_DTW_DATA) && pwfc->rxsize)
    {
        ...
        param = wfc_sdio_get_cmd_buf((uint32_t)pwfc->rxptr);
    }
    ...
    {                 
    	...
    	//WiFi Configure Cmd
        case WFC_CMD_WIFI_READY:
        {
            char is_ready;
               		
            is_ready = (wifi_firmware_is_ready()?1:0);
    ...
    {	
        wfc_sdio_free_cmd_buf((uint32_t)pwfc->rxptr);
...
{
    if ( xTaskCreate((TaskFunction_t)wfc_memq_handler, WFC_MQ_TASK_NAME, WFC_MQ_TASK_STACK_SIZE, NULL, WFC_MQ_TASK_PRIORITY, &wfc_mq_task_handle) != pdPASS )
    {
    	//printf( "WFC Task Create Fail!\r\n");
    	return;
    }	

    if ( xTaskCreate((TaskFunction_t)wfc_queue_handler, WFC_QUEUE_TASK_NAME, WFC_QUEUE_TASK_STACK_SIZE, NULL, WFC_QUEUE_TASK_PRIORITY, &wfc_queue_task_handle) != pdPASS )
    {
    	//printf( "WFC Task Create Fail!\r\n");
    	return;
    }	
...
{
	g_sem_rsp      = wifi_os_semphr_create_binary();
