...
/* register a log module in syslog to dump heap size guard info */
...
#endif /* __GNUC__ */
{
    ...
    uint32_t i = 0, xBlockSize = 0, xMiniEverTotalFree = 0U, xMiniEverTotalFreeTmp = 0U;
    ...
    printf("H_CurSys_A:%d,0x%x,%d,%d,0x%x,%d\r\n",
           xTaskGetTickCount(),
    ...
    if (xMiniEverTotalFreeTmp < xMiniEverTotalFree) {
        ...
        printf("H_PeakSys:%d,0x%x,%d,%d,0x%x,%d\r\n",
               xTaskGetTickCount(),
               (unsigned int)pvReturn,
               xwanted_size,
               xBlockSize,
    ...
    if (xTaskGetSchedulerState() == taskSCHEDULER_NOT_STARTED) {
        if (xInitStageGuard.xHeapGuardFlag == pdTRUE) {
            ...
            if (xInitStageGuard.xCurAllocSize > xInitStageGuard.xPeakAllocSize) {
                ...
                printf("H_PeakTask_A:%d,0x%x,%d,%d,Init,0x%x,%d,%d,%d\r\n",
                       xTaskGetTickCount(),
                       (unsigned int)pvReturn,
                       xwanted_size,
                       xBlockSize,
    ...
    if (xTargetCount != -1) {
        ...
        if (flag) {
            /*H_PeakTask_A:tick,addr,userSize,blockSize,taskName,lr,peakTask,curTask,peakSys */
            printf("H_PeakTask_A:%d,0x%x,%d,%d,%s,0x%x,%d,%d,%d\r\n",
                   xTaskGetTickCount(),
                   (unsigned int)pvReturn,
                   xwanted_size,
                   xBlockSize,
    ...
    if (xBlockSize >= SPECIFIC_SIZE) {
        ...
        printf("H_Specific_A:%d,0x%x,%d,%d,Init,0x%x\r\n",
               xTaskGetTickCount(),
               (unsigned int)pvReturn,
               xwanted_size,
               xBlockSize,
...
#endif /* __GNUC__ */
{
    ...
    printf("H_CurSys_F:%d,0x%x,--,%d,0x%x,%d\r\n",
           xTaskGetTickCount(),
           (unsigned int)pv,
           xBlockSize,
           (unsigned int)xLinkRegAddr,
    ...
    if (xTaskGetSchedulerState() == taskSCHEDULER_NOT_STARTED) {
        if (xInitStageGuard.xHeapGuardFlag == pdTRUE) {
            ...
            printf("H_CurTask_F:%d,0x%x,--,%d,Init,0x%x,%d,%d\r\n",
                   xTaskGetTickCount(),
                   (unsigned int)pv,
                   xBlockSize,
                   (unsigned int)xLinkRegAddr,
    ...
    if (xTargetCount != -1) {
        ...
        if (pxStatInfo->xHeapGuardFlag == pdTRUE) {
            ...
            if (flag) {
                ...
                printf("!H_CurTask_F:%d,0x%x,--,%d,%s,0x%x,%d,%d\r\n",
                       xTaskGetTickCount(),
                       (unsigned int)pv,
                       xBlockSize,
                       xHeapSizeGuardArr[xTargetCount].ucTaskName,
        ...
        printf("H_CurTask_F: not registered task: %s,0x%x\r\n", pcTaskGetTaskName(xTaskGetCurrentTaskHandle()), (unsigned int)xLinkRegAddr);
    }
#endif /* HEAP_TASK_PEAK_PROFILING */
#ifdef HEAP_SPECIFIC_SIZE_USER_STATISTIC
    if (xBlockSize >= SPECIFIC_SIZE) {
        if (flag) {
            ...
            printf("!H_Specific_F:%d,--,0x%x,%d,%s,0x%x\r\n",
                   xTaskGetTickCount(),
                   (unsigned int)pv,
                   xBlockSize,
                   xHeapSizeGuardArr[xTargetCount].ucTaskName,
...
#define SINGLE_STR_SIZE 64
    ...
    if (xTaskHandle == NULL) {
        ...
        if (pxResultStr == NULL) {
            printf("allocate result buffer fail.\r\n");
            return;
        } else {
