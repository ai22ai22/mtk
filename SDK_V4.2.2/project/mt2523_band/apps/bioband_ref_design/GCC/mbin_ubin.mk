...
APP_FILES 	+= $(APP_PATH)/../../../../driver/board/mt2523_band/keypad/src/keypad_custom.c
...
ifeq ($(CHECK_DEP),full)
	...
	@if [ -s "$(OUTPATH)/decoupling/m_reference_u_func.txt" ];then \
		echo "invalid reference, please check $(OUTPATH)/decoupling/m_reference_u_func.txt."; \
	elif [ -s "$(OUTPATH)/decoupling/m_reference_u_data.txt" ];then \
		echo "invalid reference, pleae to check $(OUTPATH)/decoupling/m_reference_u_data.txt."; \
	else \
