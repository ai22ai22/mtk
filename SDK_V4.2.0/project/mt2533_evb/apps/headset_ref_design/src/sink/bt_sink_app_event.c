    ...
    switch (event_id) {
        ...
        case BT_SINK_SRV_EVENT_HF_CALL_LIST_INFORMATION:
            bt_sink_app_report("[Sink] call list information, address:0x%2x,0x%2x,0x%2x,0x%2x,0x%2x,0x%2x, final:%d",
                event->call_list.bt_addr[5],
                event->call_list.bt_addr[4],
                event->call_list.bt_addr[3],
                event->call_list.bt_addr[2],
                event->call_list.bt_addr[1],
                event->call_list.bt_addr[0],
