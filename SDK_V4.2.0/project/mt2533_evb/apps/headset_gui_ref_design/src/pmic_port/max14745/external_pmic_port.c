...
void external_pmic_ctrl_power(unsigned char driver_name, unsigned char enable)
{
    switch (driver_name) {
        ...
        case DRIVER_USB:
            ...
            pmic_ctrl_power_maxim(EXT_PMIC_MAXIM_LDO3, enable);
            break;
...
unsigned char external_pmic_set_vcore_voltage(unsigned char lock, unsigned char vcore_vosel)
{
    ...
    external_pmic_voltage_t vol_index;

    if (init == 0) {
        old_vcore = external_pmic_get_vcore_voltage();
    ...
    if (vol_index != old_vcore) {
        old_vcore = vol_index;
        /*Set Vcore Voltege*/
        external_pmic_vosel_config(DRIVER_DVFS, vol_index);
    }
    return (vol_index);
}
...
unsigned char external_pmic_get_vcore_voltage(void)
{
    ...
    return ((external_pmic_voltage_t)vcore);
}
