...
static void mdns_txtrecord_info()
{
    ...
    char valueStr[20] = {0};
    os_strncpy(valueStr, value, valueLen);
    LOG_I(mdns_example, "TXTRecordGetValuePtr [os] value: %s strlen=%d valueLen=%d\n",
            valueStr, os_strlen(valueStr), valueLen);
}
...
static DNSServiceErrorType mdns_update_txtrecord()
{
    ...
    ret = TXTRecordSetValue(&txtRecord, KEY_SEED, os_strlen(seedString), seedString);
...
static DNSServiceErrorType mdns_publish_service()
{
    ...
    // Add Device ID
    err = TXTRecordSetValue(&txtRecord, KEY_DEVICEID, os_strlen(device), device);
    require_noerr(err, exit);

    // Add Seed Number
    os_memset(seedString, 0, 16);
    os_snprintf(seedString, 16, "%d", seed);
    err = TXTRecordSetValue(&txtRecord, KEY_SEED, os_strlen(seedString), seedString);
    require_noerr(err, exit);

    // Add OS
    err = TXTRecordSetValue(&txtRecord, KEY_OS, os_strlen(os), os);
    require_noerr(err, exit);
