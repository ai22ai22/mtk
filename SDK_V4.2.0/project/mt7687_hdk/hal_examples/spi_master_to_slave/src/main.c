...
#define SPI_TEST_FREQUENCY           12000000
...
static void spim_receive_data_from_spis(void)
{
    ...
    for (i = 0; i < SPI_TEST_DATA_SIZE / 4; i++) {
        ...
        if (data != SPI_TEST_RX_DATA_PATTERN) {
            log_hal_error("spim_read_slave_data fail: i=%d, data=%x\r\n", i, data);
            return;
        }
    }
...
/**
*@brief  Example of sending and receiving data between SPI master and SPI slave.
*@param  None.
*@return None.
*/
static void spi_master_transfer_data_two_boards_example(void)
{
    ...
    printf("---spi_master_transfer_data_two_boards_example begin---\r\n\r\n");
    ...
    if (HAL_SPI_MASTER_STATUS_OK != hal_spi_master_init(SPI_TEST_MASTER, &spi_config)) {
        log_hal_error("hal_spi_master_init fail\r\n");
        return;
    } else {
    ...
    if (HAL_SPI_MASTER_STATUS_OK != hal_spi_master_deinit(SPI_TEST_MASTER)) {
        log_hal_error("hal_spi_master_deinit fail\r\n");
        return;
    }
    ...
    printf("---spi_master_transfer_data_two_boards_example end---\r\n");
}
