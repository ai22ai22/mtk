...
static uint8_t i2c_receive_data[I2C_SEND_DATA_NUMBER] = {0};
static volatile uint8_t is_transaction_finish = 0;
static volatile uint8_t is_dma_transaction_finish = 0;
static volatile uint32_t test_length = 8; /* One single write should be a maximum size of 16bytes */
...
static void i2c_control_EEPROM_with_dma_example()
{
    ...
    if (HAL_I2C_STATUS_OK != hal_i2c_master_init(i2c_port, &i2c_init)) {
        printf("hal_i2c_master_init failed!\r\n");
        return;
    }
    /* Register callback function */
    if (HAL_I2C_STATUS_OK != hal_i2c_master_register_callback(i2c_port, i2c_callback, NULL)) {
        printf("hal_i2c_master_register_callback failed!\r\n");
        return;
    }
    if (HAL_I2C_STATUS_OK != hal_i2c_master_send_dma(i2c_port, I2C0_EEPROM_SLAVE_ADDRESS, i2c_send_data, test_length)) {
    ...
    if (HAL_I2C_STATUS_OK != hal_i2c_master_deinit(i2c_port)) {
        printf("hal_i2c_master_deinit failed\r\n");
        return;
    }
    ...
    if (HAL_I2C_STATUS_OK != hal_i2c_master_send_dma(i2c_port, I2C0_EEPROM_SLAVE_ADDRESS, i2c_send_data, 1)) {
        printf("I2C send failed\r\n");
        return;
    } else {
        printf("I2C send OK\r\n");
    ...
    if (HAL_I2C_STATUS_OK != hal_i2c_master_receive_dma(i2c_port, I2C0_EEPROM_SLAVE_ADDRESS, i2c_receive_data, test_length - 1)) {
        printf("I2C receive failed\r\n");
        return;
    } else {
        printf("I2C receive OK\r\n");
    }
    ...
    /*Step5: Compare the write data and the read back data to check if data transfer is successful.*/
