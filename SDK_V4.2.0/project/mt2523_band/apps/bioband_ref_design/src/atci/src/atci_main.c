...
void atci_processing_n_spp(void)
{
    /* Queue number */
    ...
    while (1) {
        ...
        while (notify_queue_msg_num > 0) {
            ...
            ret = atci_response_callback((atci_response_t *)(msg_queue_data.msg_data));
...
#if 0
atci_status_t atci_local_deinit_1(void)
{
    /* Queue number */
    uint16_t            common_queue_msg_num = 0;
    uint16_t            notify_queue_msg_num = 0;
...
atci_status_t atci_deinit_1(void)
{
    /* Init */
    atci_status_t ret = ATCI_STATUS_ERROR;
    ...
    ret = atci_spp_deinit(g_atci_spp_port);
