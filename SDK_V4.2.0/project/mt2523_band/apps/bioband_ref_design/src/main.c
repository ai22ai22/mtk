...
static hal_mpu_region_config_t mpu_cfg_tbl[] = {
    /* mpu_region_address, mpu_region_size, mpu_region_access_permission, mpu_subregion_mask, mpu_xn */
    {VECTBL_BASE, HAL_MPU_REGION_SIZE_256B, HAL_MPU_READONLY, 0x0, FALSE}, /**< VECTOR TABLE IN RAM AS X/RO/256B*/
    {BL_BASE, HAL_MPU_REGION_SIZE_4MB, HAL_MPU_READONLY, 0x0, FALSE} /**< SF AS X/RO/4MB */
};

static hal_mpu_config_t mpu_config = {
    /* PRIVDEFENA, HFNMIENA */
    TRUE, TRUE
};
...
static void mpu_init(void)
{
    ...
    region_number = sizeof(mpu_cfg_tbl) / sizeof(mpu_cfg_tbl[0]);
    if (region_number > HAL_MPU_REGION_MAX) {

    }

    hal_mpu_init(&mpu_config);
    for (region = 0; region < region_number; region++) {
        hal_mpu_region_configure(region, &mpu_cfg_tbl[region]);
