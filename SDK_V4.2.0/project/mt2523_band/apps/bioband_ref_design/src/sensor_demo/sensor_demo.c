...
static TimerHandle_t bpDataTimer; /* possibly more timer for different polling rate needed */

static void sensor_dump_bp_TimerCallback(void *arg);
static void sensor_set_dump_data_timer(void);
static void sensor_stop_dump_data_timer(void);
    ...
    switch (output->data[0].sensor_type) {
            ...
            if (output->data[0].sleep_data_t.feature == NULL) {
                ...
                if (output->data_exist_count > 0) {
                    ...
                    while (i < output->data_exist_count) {
                        if (output->data[i].sleep_data_t.state == -11) { /*INVOKE_STAGER*/
                        	// unsubscribe tracker
                        	// subscribe stager
                        } else if (output->data[i].sleep_data_t.state == -12) { /*INVOKE_TRACKER*/
                        	// unsubscribe stager
                        	// subscribe tracker
                        } else {
        ...
        case SENSOR_TYPE_BLOOD_PRESSURE_MONITOR: {
            if (output->data[0].blood_pressure_t.pwtt == NULL) {
                ...
                {
                    uint32_t bl_buff[5];
                    bl_buff[0] = output->data[0].blood_pressure_t.sbp;
                    bl_buff[1] = output->data[0].blood_pressure_t.dbp;
                    bl_buff[2] = output->data[0].blood_pressure_t.bpm;
                    bl_buff[3] = output->data[0].blood_pressure_t.status;
                    bl_buff[4] = output->data[0].time_stamp;
                    sensor_dump_raw_data_in_bt_log_format(SENSOR_TYPE_BLOOD_PRESSURE_MONITOR, 0, (int32_t *)bl_buff, 5);
                }
                #endif
                send_sensor_data_via_btspp(
                    SENSOR_SPP_DATA_MAGIC,
...
void enable_sleep_tracker(void)
{
    ...
    if (sleep_triggered == 0) {
        subscription = sensor_subscribe_sensor(&sleep_subscriber);
        if (subscription < 0) {
            SENSOR_DEMO_LOGI("sleep subscription fail\r\n");
        } else {
            /* reset bt buffer index of input start */
            acc_buff_count = 0;
            ppg1_buff_count = 0;
            ...
            SENSOR_DEMO_LOGI("sleep triggered\r\n");
            sleep_tracker_seq = 0;
            sleep_tracker_buff_count = 0;
            sleep_triggered = 1;
        }
        ...
        if (unsubscription < 0) {
            SENSOR_DEMO_LOGI("sleep unsubscription fail\r\n");
        } else {
            sleep_triggered = 0;
            SENSOR_DEMO_LOGI("sleep unsubscribed\r\n");
        }
    }
}
