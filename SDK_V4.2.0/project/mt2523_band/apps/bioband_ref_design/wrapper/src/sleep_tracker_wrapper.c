...
typedef int (*func1_ptr)(int, int, uint8_t);
...
int  hr_fet_ext_fota_wrapper(int ppg_bisi, int timestamp, uint8_t sensor_type)
{
    func1_ptr temp;
    temp = (func1_ptr)func_entry_tbl[hr_fet_ext_fota_wrapper_idx];
    PRINT_FUNC_ADDR(temp);
    return temp(ppg_bisi, timestamp, sensor_type);
}
