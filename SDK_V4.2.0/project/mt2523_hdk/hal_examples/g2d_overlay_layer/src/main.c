...
int g2d_test(void)
{
    ...
    log_hal_info("Address:\r\n");
    log_hal_info("wallpaper = 0x%08X\r\n", wallpaper);
    log_hal_info("brandmark = 0x%08X\r\n", brandmark);
    log_hal_info("font_layer = 0x%08X\r\n", font_layer);
    log_hal_info("frame_buffer = 0x%08X\r\n", frame_buffer);
    ...
    BSP_LCD_Init(0x0000);
    BSP_LCD_GetParam(LCM_IOCTRL_QUERY__LCM_WIDTH, &screen_width);
    BSP_LCD_GetParam(LCM_IOCTRL_QUERY__LCM_HEIGHT, &screen_height);
    BSP_Backlight_init();

    log_hal_info("LCD width = %d, LCD height = %d\r\n", screen_width, screen_height);
    ...
    BSP_LCD_ConfigROI(&lcd_parameters);
    ...
    log_hal_info("G2D test done.\r\n");

    BSP_LCD_UpdateScreen(lcd_parameters.target_start_x , lcd_parameters.target_start_y ,lcd_parameters.target_end_x, lcd_parameters.target_end_y);
