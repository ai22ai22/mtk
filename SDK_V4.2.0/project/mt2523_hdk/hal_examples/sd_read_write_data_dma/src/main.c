...
static void sd_read_write_data_dma_example(void)
{
    ...
    while (retry < 3) {
        ...
        if (HAL_SD_STATUS_OK != hal_sd_write_blocks_dma(HAL_SD_PORT_0, sd_write_mutil_buffer, 0x50, write_block_number)) {
            if (HAL_SD_STATUS_OK == hal_sd_init(HAL_SD_PORT_0, &sd_cfg)) {
                retry++;
            } else {
    ...
    if (3 <= retry) {
        log_hal_error("SD write (dma mode) failed!\r\n");
    }

    /*read with dma mode*/
    retry = 0;
    while (retry < 3) {
        if (HAL_SD_STATUS_OK != hal_sd_read_blocks_dma(HAL_SD_PORT_0, sd_read_mutil_buffer, 0x50, read_block_number)) {
            if (HAL_SD_STATUS_OK == hal_sd_init(HAL_SD_PORT_0, &sd_cfg)) {
                retry++;
            } else {
                log_hal_error("SD read (dma mode) failed!\r\n");
                break;
            }
        } else {
            break;
