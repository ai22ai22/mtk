...
typedef enum {
    GPRS_ACTION_TYPE_ACTIVATE   = 1,
    GPRS_ACTION_TYPE_DEACTIVATE = 2,
    GPRS_ACTION_TYPE_SETNETINFO = 3,
...
/* res: GPRS_RET_OK, activate/deactivate GPRS successfully.
  ...
  */
typedef void (* gprs_result_callback)(gprs_ret_t res, void *user_data);

typedef void (* gprs_reset_action_callback)(void);

typedef void (* gprs_noti_callback)(gprs_noti_type_enum noti_type, void *user_data);
...
extern gprs_ret_t gprs_is_gprs_activated(void);
...
/* Can't call this API again before the result of previous invokation is returned. 
  ...
  *            Otherwise, failed.
  */	
extern gprs_ret_t gprs_activate_with_cid(gprs_pdp_context_id_enum cid, gprs_reset_action_callback cb);
...
/* Can't call this API again before the result of previous invokation is returned. 
  ...
  *            Otherwise, failed.
  */
extern gprs_ret_t gprs_deactivate_with_cid(gprs_pdp_context_id_enum cid, gprs_reset_action_callback cb);

/* After GPRS is activated, user should call this API to set network information.
  * Return: OK, done successfully.
  *            WOULDBLOCK, wait for callback to get the result.
  *            Otherwise, failed.
  * Only when res in cb is OK, will socket be able to work.  
  */
extern gprs_ret_t gprs_set_nw_info_with_cid(gprs_pdp_context_id_enum cid, gprs_reset_action_callback cb);
