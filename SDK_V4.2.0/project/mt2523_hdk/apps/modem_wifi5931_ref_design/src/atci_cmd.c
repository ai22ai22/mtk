...
void atci_gprs_deactive_result(gprs_ret_t res, void *user_data);

void atci_gprs_active_result(char* result, void* userdata)
{
    atci_response_t resonse;
    
    LOGI("activate result");
    
    memset(&resonse, 0, sizeof(atci_response_t));
    sprintf((char *)resonse.response_buf,"activate result\r\n");
    resonse.response_len = strlen((char *)resonse.response_buf);
    resonse.response_flag |= ATCI_RESPONSE_FLAG_QUOTED_WITH_LF_CR;   
    atci_send_response(&resonse);
    
    memset(&resonse, 0, sizeof(atci_response_t));
    strcpy((char *)resonse.response_buf, result);
    resonse.response_len = strlen((char *)resonse.response_buf);
    resonse.response_flag |= ATCI_RESPONSE_FLAG_QUOTED_WITH_LF_CR;   
    atci_send_response(&resonse);
}
...
void atci_gprs_deactive_result(gprs_ret_t res, void *user_data)
{
    ...
    LOGI("deactive result");
    
    memset(&resonse, 0, sizeof(atci_response_t));
    if (GPRS_RET_OK == res) {
        sprintf((char *)resonse.response_buf,"deactivate success\r\n");
    } else {
        sprintf((char *)resonse.response_buf,"deactivate fail\r\n");
    }
    resonse.response_len = strlen((char *)resonse.response_buf);
    resonse.response_flag |= ATCI_RESPONSE_FLAG_QUOTED_WITH_LF_CR;   
    atci_send_response(&resonse);
       ...
       case ATCI_CMD_MODE_READ:    // rec: AT+MD?
        ...
        case ATCI_CMD_MODE_EXECUTION: // rec: AT+MD=<p1>  the handler need to parse the parameters
            ...
            if (strstr((char *) parse_cmd->string_ptr, "AT+MD=0") != NULL) {
                ...
                nw_atci_set_deactivate_result_callback(atci_gprs_deactive_result);
