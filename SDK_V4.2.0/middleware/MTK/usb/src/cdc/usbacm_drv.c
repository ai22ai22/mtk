...
#ifdef MTK_PORT_SERVICE_ENABLE
kal_uint8 USB_PORT[SERIAL_PORT_DEV_MAX];
#else
kal_uint8 USB_PORT[2];
#endif
...
void USB_UnMask_COM_Intr(kal_uint32 port)
{
...
void USB_Mask_COM_Intr(kal_uint32 port)
{
    kal_uint32 usb_port = port;
    if (g_UsbACM[usb_port].rxpipe != NULL) {
...
static hal_usb_dma_handler_t USB_COM_DMA_Callback(kal_uint32 port)
{
...
void USB2UART_Dafault_Tx_Callback(kal_uint32 port)
{
    kal_uint32 usb_port = USB_PORT[port];
...
void USB2UART_Dafault_Rx_Callback(kal_uint32 port)
{
    kal_uint32 usb_port = port;//USB_PORT[port];
...
*/
static kal_uint32 USB2UART_Check_Transmit_Data(kal_uint32 port, kal_uint32 *addr)
{
    kal_uint32 usb_port = USB_PORT[port];
    kal_uint32 length = 0;
...
static void USB2UART_Update_Transmit_Data(kal_uint32 port)
{
    kal_uint32 usb_port = USB_PORT[port];
...
*/
void USB2UART_Polling_Flush_Transmit_Data(kal_uint32 port)
{
#if 0
    kal_uint32 usb_port = USB_PORT[port];
    kal_uint32 addr;
...
void USB2UART_DMATransmit(kal_uint32 port)
{
    kal_uint32 usb_port = USB_PORT[port];
    kal_uint32 addr = 0;
    kal_uint32 length = 0;
    ...
    if (length != 0) {
        ...
        if (usb_dma_setup == KAL_TRUE) {
            ...
            hal_usb_start_dma_channel(ep_num, HAL_USB_EP_DIRECTION_TX, HAL_USB_EP_TRANSFER_BULK, (void *)addr, length, dma_callback, KAL_FALSE, HAL_USB_DMA1_TYPE);
        }
...
void USB2UART_Tx_DMA_Callback(kal_uint32 port)
{
    kal_uint32 usb_port = USB_PORT[port];
    kal_uint32 savedMask;
...
void USB_Acm_CommIf_Create(uint8_t port, void *ifname)
{
    ...
    g_UsbACM[port].comm_if_info->interface_name_ptr = (kal_char *)ifname;
...
void USB_Acm_DataIf_Create(uint8_t port, void *ifname)
{
    ...
    /* Record interface name and interface descriptor length */
    g_UsbACM[port].data_if_info->interface_name_ptr = (kal_char *)ifname;
...
void USB_Acm_DataIf_Speed_Reset(uint8_t port, bool b_other_speed)
{
    ...
    temp_max_size = USB_Speed_Reset_Packet_Size((kal_bool)b_other_speed);
...
/*
    ...
    It must not be used in normal time
*/
void USB2UART_Polling_Recv_Data(kal_uint32 port)
{
    kal_uint32 usb_port = USB_PORT[port];
...
void USB_Acm_BulkOut_Hdr(uint8_t port)
{
    ...
    if (USB2UARTPort[usb_port].initialized == KAL_FALSE) {
        return;
    }
    ...
    hal_usb_start_dma_channel(g_UsbACM[port].txpipe->byEP, HAL_USB_EP_DIRECTION_TX, HAL_USB_EP_TRANSFER_BULK, data, nCount, NULL, KAL_TRUE, HAL_USB_DMA1_TYPE);
#endif
}
