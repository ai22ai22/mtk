...
/*__align(4)*/ ATTR_RWDATA_IN_NONCACHED_RAM static kal_uint8 senseDataProtect[] = {0x70, 0x00, 0x07, 0x00, 0x00, 0x00, 0x00, 0x0A, 0x00,
...
void USB_Ms_State_Main(USB_Ms_State event, usb_ms_rec_done_conf_struct *local_para_ptr)
{
    ...
    if ((event != MSG_ID_USB_MSDRV_RESET_IND) && ((g_UsbMS.usb_reset == KAL_TRUE) || (g_UsbMS.ms_is_reset == KAL_TRUE))) { //receive reset
...
static void USB_Ms_Cmd_Unknown(UsbMs_CBW *CBW)
{
    ...
    if ((CBW->dCBWDataTransferLength != 0) && (CBW->bmCBWFlags & USBMS_DIR_IN)) {
        ...
        } while ((bEPEmpty == KAL_FALSE) && (count != (kal_uint8)USBMS_UNKNOW_CMD_TIMEOUT) && (USB_Get_Device_Type() != USB_UNKOWN));
    }

    if ((count != (kal_uint8)USBMS_UNKNOW_CMD_TIMEOUT) && (USB_Get_Device_Type() != USB_UNKOWN));
    {
        USB_Ms_Build_Tx_CWS(&g_UsbMS_CSW, CBW);
    }
}
                                   ...
                                   kal_uint32 search_value, kal_uint32 *p_index)
{
    ...
    if (search_value == data_items[index]) {
        ...
        if ((index < data_size) && (search_value > data_items[index]))
            //EXT_ASSERT(0, search_value, data_items[index], (kal_uint32)index);
        {
            LOG_E(hal, "EXT_ASSERT\n");
        }
    }
