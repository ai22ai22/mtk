...
void USB_Ms_BuildRx(void *addr, kal_uint32 len)
{
    ...
    if ((g_UsbMS.usb_reset == KAL_FALSE) && (g_UsbMS.ms_is_reset == KAL_FALSE)) {
        ...
        hal_usb_start_dma_channel(g_UsbMS.rxpipe->byEP, HAL_USB_EP_DIRECTION_RX, HAL_USB_EP_TRANSFER_BULK, (void *)addr, len, USB_Ms_Rx_DMA_Callback, KAL_FALSE, HAL_USB_DMA1_TYPE);
    }
...
void USB_Ms_BuildTx(void *addr, kal_uint32 len)
{
    ...
    if ((g_UsbMS.usb_reset == KAL_FALSE) && (g_UsbMS.ms_is_reset == KAL_FALSE)) {
        ...
        if (len != 13) {
            ...
            hal_usb_start_dma_channel(g_UsbMS.txpipe->byEP, HAL_USB_EP_DIRECTION_TX, HAL_USB_EP_TRANSFER_BULK, (void *)addr, len, USB_Ms_Tx_DMA_Callback, KAL_FALSE, HAL_USB_DMA1_TYPE);
        } else {
            /* DMA running state is NOT cleared by USB_Ms_Tx_DMA_Callback */
            hal_usb_start_dma_channel(g_UsbMS.txpipe->byEP, HAL_USB_EP_DIRECTION_TX, HAL_USB_EP_TRANSFER_BULK, (void *)addr, len, NULL, KAL_FALSE, HAL_USB_DMA1_TYPE);
        }
    }
...
void USB_Ms_If_Create(void *ifname)
{
    ...
    hal_usb_get_dma_channel(ep_in_id, ep_out_id, HAL_USB_EP_DIRECTION_TX, KAL_TRUE);
}
...
{
    hal_usb_release_dma_channel(g_UsbMS.txpipe->byEP, g_UsbMS.rxpipe->byEP, HAL_USB_EP_DIRECTION_TX, KAL_TRUE);
    USB_Init_Ms_Status();
	USB_Free_Memory();
...
static void USB_Ms_Rx_DMA_Callback(void)
{
    ...
    if (USB_Get_Device_State() == DEVSTATE_CONFIG) {
        ...
        if ((USB_Get_Device_State() == DEVSTATE_CONFIG) && (g_UsbMS.ms_is_reset == KAL_FALSE)) {
            USB_Ms_State_Main(MSG_ID_USB_MSDRV_REC_DONE_CONF, rec_done_conf);
        }
    }
}
...
static void USB_Ms_Cmd_Reset(void)
{
    ...
    hal_usb_clear_tx_endpoint_fifo(g_UsbMS.txpipe->byEP, HAL_USB_EP_TRANSFER_BULK, KAL_TRUE);

    hal_usb_clear_rx_endpoint_fifo(g_UsbMS.rxpipe->byEP, HAL_USB_EP_TRANSFER_BULK, KAL_TRUE);
    ...
    switch (pcmd->bRequest) {
        case USB_MS_RESET:
            if ( (pcmd->bmRequestType != USB_CMD_CLASSIFOUT) || (pcmd->wValue != 0) ) {
                ...
                hal_usb_set_endpoint_stall(g_UsbMS.txpipe->byEP, HAL_USB_EP_DIRECTION_TX, KAL_FALSE);
                hal_usb_set_endpoint_stall(g_UsbMS.rxpipe->byEP, HAL_USB_EP_DIRECTION_RX, KAL_FALSE);
            }
            break;
...
static void USB_Ms_BulkOut_Hdlr(void)
{
    ...
    if (nCount == 31) {
        ...
        /* Error handling */
        LOG_E(hal, "USBMS_RX_WRONG_CBW_LENGTH\n");

        g_UsbMS.cbw_is_vaild = KAL_FALSE;
        hal_usb_set_endpoint_stall(g_UsbMS.txpipe->byEP, HAL_USB_EP_DIRECTION_TX, KAL_TRUE);
        hal_usb_set_endpoint_stall(g_UsbMS.rxpipe->byEP, HAL_USB_EP_DIRECTION_RX, KAL_TRUE);
    }
}
