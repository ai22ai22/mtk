...
#ifdef __BT_SINK_SRV_DUAL_SCO_SUPPORT__
static bt_status_t bt_sink_srv_hf_set_audio_status(uint32_t handle, bt_hfp_audio_status_t status);
#endif

#ifndef __BT_SINK_SRV_DUAL_SCO_SUPPORT__
void bt_sink_srv_hf_audio_connect_callback(void *parameter)
{
    bt_sink_srv_hf_context_t *device = (bt_sink_srv_hf_context_t *)parameter;

    if (!(device->flag & BT_SINK_SRV_HF_FLAG_SCO_ACTIVE)) {
        bt_sink_srv_report("[Sink][HF][MP] audio to HF, handle:0x%x", device->handle);
        bt_hfp_audio_transfer(device->handle, BT_HFP_AUDIO_TO_HF);
    }
}
#endif /* __BT_SINK_SRV_DUAL_SCO_SUPPORT__ */
...
#else /* __BT_SINK_SRV_DUAL_SCO_SUPPORT__ */
    bt_sink_srv_report("[Sink][HF][MP] set highlight device, prev:%x, next:%x",
                       g_sink_srv_hf_hightlight_p, device);

    if (NULL != g_sink_srv_hf_hightlight_p && g_sink_srv_hf_hightlight_p != device) {
        if (g_sink_srv_hf_hightlight_p->flag & BT_SINK_SRV_HF_FLAG_SCO_ACTIVE) {
            bt_sink_srv_report("[Sink][HF][MP] audio to AG, handle:0x%x", g_sink_srv_hf_hightlight_p->handle);
            bt_hfp_audio_transfer(g_sink_srv_hf_hightlight_p->handle, BT_HFP_AUDIO_TO_AG);
        }

        if (NULL != device && BT_HFP_CODEC_TYPE_NONE != device->codec) {
            bt_sink_srv_timer_start(BT_SINK_SRV_TIMER_HF_AUDIO_CONN,
                                    BT_SINK_SRV_HF_AUDIO_CONN_DURATION,
                                    bt_sink_srv_hf_audio_connect_callback,
                                    (void *)device);
        }
    }
    g_sink_srv_hf_hightlight_p = device;
