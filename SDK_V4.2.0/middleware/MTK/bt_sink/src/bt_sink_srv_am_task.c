...
#ifdef __BT_AWS_SUPPORT__
    hal_audio_sampling_rate_t sampling_rate = HAL_AUDIO_SAMPLING_RATE_16KHZ;
#endif
            ...
            case BT_CODEC_MEDIA_AWS_CHECK_CLOCK_SKEW: {
                if (BT_AWS_CODEC_TYPE_SBC == g_aws_codec_type) {
                    switch (g_aws_sample_rate) {
                        case 8:
                            sampling_rate = HAL_AUDIO_SAMPLING_RATE_16KHZ;
                            break;
                        case 4:
                            sampling_rate = HAL_AUDIO_SAMPLING_RATE_32KHZ;
                            break;
                        case 2:
                            sampling_rate = HAL_AUDIO_SAMPLING_RATE_44_1KHZ;
                            break;
                        case 1:
                            sampling_rate = HAL_AUDIO_SAMPLING_RATE_48KHZ;
                            break;
                    ...
                    switch (g_aws_sample_rate) {
                        case 0x800:
                            sampling_rate = HAL_AUDIO_SAMPLING_RATE_8KHZ;
                            break;
                        case 0x400:
                            sampling_rate = HAL_AUDIO_SAMPLING_RATE_11_025KHZ;
                            break;
                        case 0x200:
                            sampling_rate = HAL_AUDIO_SAMPLING_RATE_12KHZ;
                            break;
                        case 0x100:
                            sampling_rate = HAL_AUDIO_SAMPLING_RATE_16KHZ;
                            break;
                        case 0x80:
                            sampling_rate = HAL_AUDIO_SAMPLING_RATE_22_05KHZ;
                            break;
                        case 0x40:
                            sampling_rate = HAL_AUDIO_SAMPLING_RATE_24KHZ;
                            break;
                        case 0x20:
                            sampling_rate = HAL_AUDIO_SAMPLING_RATE_32KHZ;
                            break;
                ...
                bt_sink_srv_audio_clock_calibrate(g_prA2dp_sink_handle, sampling_rate);
                break;
            }
            #endif /* __BT_AWS_SUPPORT__ */
	...
	if (external_dsp_is_power_on()) {
        ...
        for (i = 0; i < EXTERNAL_DSP_REGISTER_SENDING_PATH_CUSTOMER_REGISTER_AMOUNT; i++) {
            if(am_speech_eaps->voice_parameter.voice_wb_band.voice_wb_parameter_with_external_dsp[0].voice_wb_enhancement_parameter.external_dsp_sending_path_register_info[i].need_to_download_flag) {
                ...
                bt_sink_srv_report("[Sink][AM][EXTDSP]nvdm setting:i=%d, download_index=%d, download_value=0x%x", i, download_index, download_value);
                vTaskDelay(20);
            }
        }
