...
#define SENSOR_TYPE_ALL                             (SENSOR_VIRTUAL_TYPE_START+13)
...
*
* Below is an example of the pedometer fusion algorithm (requiring accelerometer of 50Hz sampling rate) registration.
* The steps are shown below:
*  - 1. Prepare the #sensor_descriptor_t for the pedometer.
*  - 2. Set the required input sensor.
*  - 3. Register the pedometer fusion algorithm using #sensor_fusion_algorithm_register_type().
*  - 4. Register the data buffer using #sensor_fusion_algorithm_register_data_buffer(). Please refer to sensor_manager_t in [sdk_root]/middleware/MTK/sensor_subsys/src/sensor_manager.c for data buffer allocation.
*  - Sample code:
*    @code
...
*  - Step1: Configure GPIO pins for sensor hardware connection. For more information, please refer to [sdk_root]/project/mt2523_sdk/apps/sensor_subsys_accelerometer/inc/ept_gpio_drv.h and bsp_sensor_peripheral_init
...
*  - Step4: Call the initialization function (bma255_sensor_subsys_init()) in bsp_sensor_peripheral_init().
*  - Sample code:
*    @code
