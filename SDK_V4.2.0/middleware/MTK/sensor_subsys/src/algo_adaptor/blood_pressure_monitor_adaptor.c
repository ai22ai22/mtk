...
int32_t bp_output_calibration_data[6] = {1,2,3,4,5,6}; //20161125 init value for test use, actually calibration data is come from algo.
...
static int32_t get_blood_pressure_monitor_result(sensor_data_t *const output)
{
...
#if BP_GET_ECG_ERROR
    {
        ...
        if (num_pwtt_out!=0 ) {
            ...
            bp_data->blood_pressure_t.numPwtt = 6;
            bp_data->blood_pressure_t.status = 6;
