...
static int32_t set_sleep_data(const sensor_data_t *input_list, void *reserve)
{
    ...
    if(input_list->data->sensor_type == SENSOR_TYPE_ACCELEROMETER){
      ...
      result_count = check_sleep_results(); //get results      
      fet_count    = check_sleep_motion_fet(); //get features
      
      
      if((result_count>0) || (fet_count>0)) {
    ...
    if((input_list->data->sensor_type == SENSOR_TYPE_BIOSENSOR_BISI) || 
      input_list->data->sensor_type == SENSOR_TYPE_BIOSENSOR_PPG1) {
...
int sleep_register(void)
{
  ...
  input_comp_acc.timeout = 200; /* 100 ms, default: 10Hz */
  input_comp_acc.next_input     = &input_comp_bio;

  input_comp_bio.input_type     = SENSOR_TYPE_BIOSENSOR_PPG1;
  input_comp_bio.sampling_delay = 8; /* 8 ms, default: 125Hz */ //modify to 8 when MT2511 E1 is applied.
  input_comp_bio.timeout = 64;
