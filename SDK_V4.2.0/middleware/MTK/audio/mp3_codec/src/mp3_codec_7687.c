...
static void mp3_codec_i2s_tx_isr_callback(hal_i2s_event_t event, void *user_data)
{
    ...
    uint32_t i2s_amount_space_tones_in_bytes;
    uint32_t stream_out_pcm_buffer_data_count_in_tones = 0; // note: MONO: one tone is one mono tone; STEREO: one tone is one stereo tone
    uint32_t stream_out_tone_amount = 0;
    ...
    switch (event) {
        case HAL_I2S_EVENT_DATA_REQUEST:
            ...
            for (loop_idx = 0; loop_idx < 2; loop_idx++) {
                ...
                uint32_t i2s_amount_space_tones_in_bytes = 0;
                hal_i2s_get_tx_sample_count(&i2s_amount_space_tones);
                i2s_amount_space_tones_in_bytes = i2s_amount_space_tones * 4;
