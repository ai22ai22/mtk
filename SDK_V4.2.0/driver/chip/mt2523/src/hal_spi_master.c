        ...
        void *user_data)
{
    ...
    /* Prevent SPI IRQ from rising after SPI IRQ is enabled.
     * User maybe has already called polling APIs to send or receive data.
     */
    spi_clear_irq_status(master_port);
        ...
        uint32_t size)
{
    ...
    if (true == HAL_CACHE_IS_BUFFER_CACHEABLE((uint32_t)data, size)) {
        return HAL_SPI_MASTER_STATUS_INVALID_PARAMETER;
    }
#endif
    /* check HW status before configure HW registers */
...
#ifdef HAL_CACHE_MODULE_ENABLED
    if (true == HAL_CACHE_IS_BUFFER_CACHEABLE((uint32_t)(spi_send_and_receive_config->receive_buffer), (spi_send_and_receive_config->receive_length)) ||
            true == HAL_CACHE_IS_BUFFER_CACHEABLE((uint32_t)(spi_send_and_receive_config->send_data), (spi_send_and_receive_config->send_length))
       ) {
        return HAL_SPI_MASTER_STATUS_INVALID_PARAMETER;
    }
#endif
