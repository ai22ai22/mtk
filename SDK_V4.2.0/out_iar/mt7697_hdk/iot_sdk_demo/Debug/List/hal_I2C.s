...
//    List file    =  
//        C:\LEO\myGit\MTK
//        IOT\SDK_V4.2.0\out_iar\mt7697_hdk\iot_sdk_demo\Debug\List\hal_I2C.s
//
///////////////////////////////////////////////////////////////////////////////
...
??i2c_master_wait_transaction_finish_0:
        ...
        CMP      R9,#+1
        LDR      R0,[R4, #+52]
        BNE.N    ??i2c_master_wait_transaction_finish_2
        LSLS     R0,R0,#+29
        BMI.N    ??i2c_master_wait_transaction_finish_3
        CMP      R5,#+240
        BGE.N    ??i2c_master_wait_transaction_finish_3
??i2c_master_wait_transaction_finish_4:
        ...
        LDR      R0,[SP, #+4]
        CMP      R0,#+131072
        BLS.N    ??i2c_master_wait_transaction_finish_1
        STR      R9,[SP, #+0]
        MOV      R3,R6
        ADR.W    R2,?_0
        MOVS     R1,#+179
...
??i2c_master_wait_transaction_finish_2:
        LSLS     R0,R0,#+29
        BPL.N    ??i2c_master_wait_transaction_finish_4
...
//  400     return ret;
        ...
        POP      {R4-R6,PC}       ;; return
??halI2CMasterCtrl_5:
        LDR.W    R1,??DataTable23_2  ;; 0x186a0
        UDIV     R0,R0,R1
        LSRS     R0,R0,#+2
        SUBS     R0,R0,#+1
        B.N      ??halI2CMasterCtrl_4
??halI2CMasterCtrl_6:
        LDR.W    R1,??DataTable23_3  ;; 0x30d40
        UDIV     R0,R0,R1
        LSRS     R0,R0,#+2
        SUBS     R0,R0,#+1
        B.N      ??halI2CMasterCtrl_4
??halI2CMasterCtrl_7:
        LDR.W    R1,??DataTable23_4  ;; 0x493e0
        UDIV     R0,R0,R1
        LSRS     R0,R0,#+2
        SUBS     R0,R0,#+1
        B.N      ??halI2CMasterCtrl_4
??halI2CMasterCtrl_8:
        LDR.W    R1,??DataTable23_5  ;; 0x61a80
        UDIV     R0,R0,R1
        LSRS     R0,R0,#+2
        SUBS     R0,R0,#+1
        B.N      ??halI2CMasterCtrl_4
...
??halI2CGeneralRW_18:
        UXTH     R6,R6
        CMP      R6,R11
        BLT.N    ??halI2CGeneralRW_17
//  898         u2RxResidual = u2RxResidual - u2RxLen;
        SUB      R8,R8,R11
        UXTH     R8,R8
??halI2CGeneralRW_16:
        ORRS     R0,R8,R7
        BEQ.N    ??halI2CGeneralRW_19
        LDR      R0,[R10, #+52]
        LSLS     R0,R0,#+29
        BPL.N    ??halI2CGeneralRW_20
        MOVS     R0,#+1
        B.N      ??halI2CGeneralRW_1
??halI2CGeneralRW_20:
        MOV      R0,R10
        ...
        BL       halI2CTxFIFOSpace
        ORRS     R1,R0,R11
        BEQ.N    ??halI2CGeneralRW_16
        CMP      R7,R0
        BGE.N    ??halI2CGeneralRW_21
        MOV      R0,R7
??halI2CGeneralRW_21:
        MOVS     R1,#+0
        B.N      ??halI2CGeneralRW_22
??halI2CGeneralRW_23:
        LDRB     R2,[R5], #+1
        STR      R2,[R10, #+60]
        ADDS     R1,R1,#+1
??halI2CGeneralRW_22:
        UXTH     R1,R1
        CMP      R1,R0
        BLT.N    ??halI2CGeneralRW_23
        SUBS     R7,R7,R0
        UXTH     R7,R7
        CMP      R8,R11
        BGE.N    ??halI2CGeneralRW_24
        MOV      R11,R8
??halI2CGeneralRW_24:
        MOVS     R6,#+0
        B.N      ??halI2CGeneralRW_18
//  899 
//  900     }
//  901 
...
??halI2CChkStatus_5:
        UXTB     R6,R6
        CMP      R6,R1
        BGE.N    ??halI2CChkStatus_7
//  994         if (u4Val & (0x1 << i)) {
        MOV      R2,R0
        LSRS     R2,R2,R6
        LSLS     R2,R2,#+31
        BPL.N    ??halI2CChkStatus_6
...
// 1045     pI2CHalfGDMATypeDef = (IOT_I2C_HALF_GDMA_TypeDef *)(CM4_DMA_BASE + (ucGDMAIdx * 0x100) + I2C_GDMA_DMAN_WPPT_OFFSET);
        LDR.N    R0,??DataTable23_6  ;; 0x83010008
        ADD      R0,R0,R7, LSL #+8
// 1046 
...
// 1090         u4Val = (u4Val | BIT(2));
        LDR.N    R2,??DataTable23_7  ;; 0xfffbfff7
        ANDS     R1,R2,R1
        ORR      R1,R1,#0x4
        B.N      ??halI2CGDMACtrl_6
// 1091     } else {
// 1092         // Rx
...
??halI2CGDMACtrl_6:
        LDR.N    R2,??DataTable23_8  ;; 0xfffd78dc
        ANDS     R1,R2,R1
        ORR      R1,R1,#0x10
        STR      R1,[R0, #+12]
// 1115 
...
// 1190         printf("[%s](!((pI2CTypeDef->MM_STATUS) & BIT(I2C_MM_START_READY_OFFSET)))\n", __FUNCTION__);
        ADR.W    R5,`halI2CGDMANormalRead::__FUNCTION__`
        MOV      R1,R5
        ADR.W    R0,?_3
          CFI FunCall printf
        BL       printf
