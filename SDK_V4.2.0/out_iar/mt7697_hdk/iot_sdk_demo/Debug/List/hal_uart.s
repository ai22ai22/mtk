...
//    Endian       =  little
//    Source file  =  
//        C:\LEO\myGit\MTK IOT\SDK_V4.2.0\driver\chip\mt7687\src\hal_uart.c
//    Command line =  
//        "C:\LEO\myGit\MTK IOT\SDK_V4.2.0\driver\chip\mt7687\src\hal_uart.c"
...
//    List file    =  
//        C:\LEO\myGit\MTK
//        IOT\SDK_V4.2.0\out_iar\mt7697_hdk\iot_sdk_demo\Debug\List\hal_uart.s
//
///////////////////////////////////////////////////////////////////////////////
        ...
        EXTERN g_uart_port_to_irq_num
        EXTERN g_uart_sleep_handler_name
...
//  104             (uart_config->word_length > HAL_UART_WORD_LENGTH_8)) {
        ...
        LDRB     R0,[R4, #+1]
        CMP      R0,#+4
        BLT.N    ??hal_uart_is_config_valid_1
//  105         return false;
...
//  186     ticks_per_us = top_mcu_freq_get() / 1000000;
        ...
        BL       top_mcu_freq_get
        LDR.W    R1,??DataTable19_2  ;; 0xf4240
        UDIV     R0,R0,R1
//  187 
...
//  212 #ifdef HAL_SLEEP_MANAGER_ENABLED
        ...
        SECTION `.text`:CODE:NOROOT(1)
          ...
          CFI Function uart_send_handler
        THUMB
...
//  221         if (g_uart_context_table[u_port].dma_config.send_vfifo_buffer_size - length == 0) {
        ...
        LDR      R1,[SP, #+4]
        SUBS     R0,R0,R1
        BNE.N    ??uart_send_handler_1
//  222             if (g_uart_send_lock_status[u_port] == true) {
        LDRB     R0,[R4, #+2]
        CMP      R0,#+0
        BEQ.N    ??uart_send_handler_1
...
??hal_uart_init_3:
        MOV      R0,R5
          CFI FunCall hal_uart_translate_port
        BL       hal_uart_translate_port
          CFI FunCall uart_set_sleep_mode
        BL       uart_set_sleep_mode
...
//  281     uart_mask_send_interrupt(hal_uart_translate_port(uart_port));
        MOV      R0,R4
          CFI FunCall hal_uart_translate_port
        BL       hal_uart_translate_port
          CFI FunCall uart_mask_send_interrupt
        BL       uart_mask_send_interrupt
...
//  387     if (int_no < UART_MAX_PORT) {
        CMP      R6,#+2
        BGE.N    ??hal_uart_receive_polling_3
...
//  561     hal_nvic_register_isr_handler(g_uart_port_to_irq_num[uart_port], uart_interrupt_handler);
        LDR.N    R6,??DataTable19_6
        LDR.N    R1,??DataTable19_9
        LDRSB    R0,[R6, R4]
...
//  562     uart_unmask_send_interrupt(hal_uart_translate_port(uart_port));
        MOV      R0,R5
          CFI FunCall hal_uart_translate_port
        BL       hal_uart_translate_port
          CFI FunCall uart_unmask_send_interrupt
        BL       uart_unmask_send_interrupt
...
??DataTable19_2:
        ...
        DATA
??DataTable19_3:
        DC32     g_uart_frist_send_complete_interrupt
...
??DataTable19_4:
        ...
        DATA
??DataTable19_5:
        DC32     g_uart_sleep_handler_name

        SECTION `.text`:CODE:NOROOT(2)
        SECTION_TYPE SHT_PROGBITS, 0
        DATA
??DataTable19_6:
        DC32     g_uart_port_to_irq_num
