...
/* buffer pool items */
#define LOG_POOL_ITEMS (2*SYSLOG_QUEUE_LENGTH)
...
#define POLLING_MODE_DMA_READY 1
...
{
    configASSERT(device >= SERIAL_PORT_DEV_UART_0 && device <= SERIAL_PORT_DEV_UART_3);
...
int log_write(char *buf, int len)
{
    ...
    if (syslog_init_done & SYSLOG_UART_INIT_DONE)  {
        ...
        if (((portNVIC_INT_CTRL_REG & 0xff) > 0xf) || (xTaskGetSchedulerState() == taskSCHEDULER_SUSPENDED)) {
            /* Do not support direct printf in ISR or scheduler suspended context. */
            return 0;
        }
        ...
        if (syslog_init_done & SYSLOG_UART_DMA_MODE) {
            while(1) {
                ...
                configASSERT(send_data.serial_port_write_data.ret_size <= send_data.serial_port_write_data.size);
                ...
                xSemaphoreTake(tx_sem, 1000/*portMAX_DELAY*/);
            }
            ret = (send_data.serial_port_write_data.size == 0) ? len : 0;
        } else {
#if (POLLING_MODE_DMA_READY == 0)
            for (i = 0; i < len; i++) {
                log_putchar(*buf++);
            }
            ret = len;
#else
            if ((syslog_init_done & SYSLOG_PORT_OPEN_DONE) && g_log_port <= SERIAL_PORT_DEV_USB_COM2) {
                /* only uart and usb support dma polling mode */
    ...
    if (strcmp(module_name, "*") == 0) {
        ...
        while (syslog_config.filters[i]) {
            ...
            if ((print_level == PRINT_LEVEL_INFO) &&
                (strcmp(syslog_config.filters[i]->module_name, "bt_spp") == 0 ||
                 strcmp(syslog_config.filters[i]->module_name, "SPP_PORT") == 0)) {
                i++;
                continue;
            }
#endif
