...
static void amr_decoder_deocde_hisr_handler(void *data)
{
    ...
    while ((ring_buffer_get_space_byte_count(&internal_handle->stream_out_pcm_buff)) >= PCM_FRAME_SIZE) {
        ...
        // LOGI("stream_out_pcm_buff pace_byte_count=%d\n", (ring_buffer_get_space_byte_count(&internal_handle->stream_out_pcm_buff)));
        total_share_buff_read_len = handle->get_data_count(handle);
        //LOGI("total_share_buff_read_len =%d bytes\r\n",total_share_buff_read_len);
        if (total_share_buff_read_len < MAX_AMR_FRAME_SIZE) { //maximum dpacked_size+1 byte toc
            LOGW("total_share_buff_read_len < MAX_AMR_FRAME_SIZE, MAX_AMR_FRAME_SIZE=%d\r\n", MAX_AMR_FRAME_SIZE);
            break;
        }
        ...
        //LOGI("quality_bit=%x , frame_type=%x\r\n",quality_bit, frame_type);
...
static amr_decoder_status_t amr_decoder_start(amr_decoder_handle_t *handle)
{
    ...
    while ((ring_buffer_get_space_byte_count(&internal_handle->stream_out_pcm_buff)) >= PCM_FRAME_SIZE) {
        ...
        if (remain != 0) { //Data in share buffer is not enough for TOC finding
            LOGE("TOC not found!\n\r");
        ...
        if (remain != 0) {
            LOGE("Data in share buffer is not enough for decoding a frame,  need=%d, remain=%d!\n\r", dpacked_size[frame_type], remain);
            //should not be here, because we share buffer and stream out pcm buffer control
            return AMR_DECODER_STATUS_ERROR;
        }
