...
#else
ATTR_ZIDATA_IN_TCM sys_cfg_t *g_wifi_profile = NULL;
...
#else
static ATTR_ZIDATA_IN_TCM fw_image_tailer_t *n9_fw_info;
#endif
...
static int connsys_download_wifi_profile(sys_cfg_t *wifi_profile)
{
    ...
    memcpy(wifi_profile_data, wifi_profile, sizeof(sys_cfg_t));

    /* for debug */
    CONNSYS_DUMP_I(connsys, "wifi_profile_data:", wifi_profile_data, sizeof(sys_cfg_t));
    INFO(connsys, "wifi profile length is %d.", sizeof(sys_cfg_t));
...
unsigned int (*get_rx_vector)(unsigned char ucGroupVLD);
#else
#ifdef MTK_CM4_WIFI_TASK_ENABLE
...
uint8_t connsys_cli_entry(uint8_t len, char *param[])
{
