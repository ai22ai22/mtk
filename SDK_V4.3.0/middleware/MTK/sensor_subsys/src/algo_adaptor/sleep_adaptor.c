...
int32_t get_sleep_data(sensor_data_t * const output)
{
  ...
  if(txcode == FEATURE_MARK0) {    
    ...
    #if (EN_ADAPTOR_GET_MSG)
      printf("get_sleep_data() @ %d - FEATURE_MARK0\n", (int)pdata->time_stamp);
    #endif
    pdata++;
    return ret;  
  }  
...
#endif    
    #if (EN_ADAPTOR_GET_MSG)
      printf("get_sleep_data() @ %d - FEATURE_MARK1\n", (int)pdata->time_stamp);
    #endif
    pdata++;
    return ret;  
  }  
  ...
  if(txcode == FEATURE_MARK2) {
    ...
    #if (EN_ADAPTOR_GET_MSG)
      printf("get_sleep_data() @ %d - FEATURE_MARK2\n", (int)pdata->time_stamp);
    #endif
    pdata++;
    return ret;  
  }
...
int sleep_register(void)
{
  ...
  tracker_input_comp_acc.sampling_delay = 100; /* 100 ms, default: 10Hz */
  tracker_input_comp_acc.timeout        = 200; /* 100 ms, default: 10Hz */
...
int sleep_staging_register(void)
{
  ...
  staging_input_comp_acc.sampling_delay = 100; /* 100 ms, default: 10Hz */
  staging_input_comp_acc.timeout        = 200; /* 100 ms, default: 10Hz */
  ...
  staging_input_comp_bio.timeout        = 64;
  ...
  staging_input_comp_bisi.timeout        = 100;
