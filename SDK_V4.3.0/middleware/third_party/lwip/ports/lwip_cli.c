                                     ...
                                     const char *value)
{
    ...
    if (!strcmp(ifname, "st2")) {
        nvdm_write_data_item("STA", key, NVDM_DATA_ITEM_TYPE_STRING, (uint8_t *)value, strlen(value));
    } else if (!strcmp(ifname, "ap1")) {
        nvdm_write_data_item("AP", key , NVDM_DATA_ITEM_TYPE_STRING, (uint8_t *)value, strlen(value));
    } else {
        return 1;
    }
#endif
