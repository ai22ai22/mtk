...
static void audio_dsp_playback_on(uint16_t active_type, uint16_t sample_rate)
{
    ...
    *DSP_AUDIO_CUR_GAIN = 0;
    ...
    if (audio_pcm_route_isr.pcm_route_id != 0) {    /* Hook PCM route ISR */
...
#endif
    if (audio_pcm_route_isr.pcm_route_id != 0) {    /* Unhook PCM route ISR */
        uint16_t mask    = AUDIO_DSP_POST_PROCESSING_ENABLE_MASK | AUDIO_DSP_POST_PROCESSING_PCM_ROUTE_MASK;
...
uint16_t audio_service_get_frame_length(uint16_t type)
{
    uint16_t frm_len;
    switch (type) {
        case ASP_TYPE_SBC_DEC:
...
/*
    return:
        ...
        others --> Error
*/
int32_t audio_pcm_route_register_isr(isr function, void *user_data)
{
    int32_t pcm_route_id = 0;
    int32_t pcm_route_bit;
    for (pcm_route_bit = 0; pcm_route_bit < MAX_PCM_ROUTE_FUNCTIONS; pcm_route_bit++) {
        pcm_route_id = 1 << pcm_route_bit;
        if ((audio_pcm_route_isr.pcm_route_id & pcm_route_id) == 0) {
            audio_pcm_route_isr.pcm_route_id |= pcm_route_id;
            break;
        }
    }
    if (pcm_route_bit >= MAX_PCM_ROUTE_FUNCTIONS) {
        pcm_route_id = -1;   /* No available PCM route ID */
    } else {
        int32_t func_id;
        for (func_id = 0; func_id < MAX_PCM_ROUTE_FUNCTIONS; func_id++) {
            if (audio_pcm_route_isr.num[func_id] == 0) {
                audio_pcm_route_isr.num[func_id] = pcm_route_id;
                audio_pcm_route_isr.function[func_id] = function;
                audio_pcm_route_isr.userdata[func_id] = user_data;
                break;
            }
        }
        if (func_id >= MAX_PCM_ROUTE_FUNCTIONS) {
            pcm_route_id = -2;  /* PCM route registration full */
        }
    }
    return pcm_route_id;
}
