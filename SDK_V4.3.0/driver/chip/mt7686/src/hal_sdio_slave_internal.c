...
void sdio_slave_isr(hal_nvic_irq_t irq_number)
{
    ...
    if ((interrupt_status.rx0_interrupte_status_mask & 0x0f) || (interrupt_status.rx1_interrupte_status_mask & 0x0f)) {
        for (i = SDIO_SLAVE_RXQ_START; i < SDIO_SLAVE_MAX_RXQ_NUM; i++) {
            if ((interrupt_status.rx0_interrupte_status_mask & SDIO_SLAVE_RXQ_DONE(i)) ||
                    (interrupt_status.rx1_interrupte_status_mask & SDIO_SLAVE_RXQ_OWN_CLEAR(i))) {
    ...
    if (interrupt_status.tx0_interrupte_status_mask & 0xff) {
        ...
        sdio_private.sdio_property.sdio_slave_callback(HAL_SDIO_SLAVE_EVENT_TX1_DONE, sdio_slave_tx_gpd[SDIO_SLAVE_TX_QUEUE_1].word_3.length.buffer_length, sdio_private.sdio_property.sdio_slave_callback_user_data);
    }
    ...
    if (interrupt_status.sw_interrupte_status_mask) {
        ...
        sdio_private.sdio_property.sdio_slave_callback(HAL_SDIO_SLAVE_EVENT_SW_INTERRUPT, (uint32_t)interrupt_status.sw_interrupte_status_mask, sdio_private.sdio_property.sdio_slave_callback_user_data);
    }
...
void sdio_slave_nvic_set(bool enable)
{
    ...
    if (enable) {
        NVIC_EnableIRQ(SDIO_SLV_IRQn);
    } else {
        NVIC_DisableIRQ(SDIO_SLV_IRQn);
    }
}
