...
void pmu_retention_ldo_control_mt7686(pmu_control_t status){
    if(status == PMU_CONTROL_OFF){
        printf("[Retention LDO off : RTC mode]");
    }else{
        printf("[Retention LDO on : Retention ldo]");
    }
...
void pmu_efuse_calibration () {
    ...
    if (efuse_check) {
            printf("[PMU EFUSE Start]\r\n"); /*3:0*/
            if (efuse_vaild & (1 << 0)) {
                printf("RG_BGR_VBGTRIM =1\r\n");
            ...
            if (efuse_vaild & (1 << 9)) { /*14:12*/
                printf("RG_RETENTION_LDO_VOSEL =1\r\n");
            ...
            printf("[0xA20A0300:%lx]\r\n",*((volatile uint32_t *)(0xA20A0300)));
            printf("[0xA20A0304:%lx]\r\n",*((volatile uint32_t *)(0xA20A0304)));
        }
}

void pmu_trigger_wd_code_reset() {
