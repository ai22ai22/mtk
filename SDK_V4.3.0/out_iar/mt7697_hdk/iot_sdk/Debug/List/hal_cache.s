...
//    Locale       =  C
//    List file    =  
//        C:\LEO\myGit\MTK-IOT\SDK_V4.3.0\out_iar\mt7697_hdk\iot_sdk\Debug\List\hal_cache.s
//
///////////////////////////////////////////////////////////////////////////////
...
//   75     CACHE->CACHE_OP &= ~CACHE_OP_OP_MASK;
        LDR.W    R0,??DataTable14_1  ;; 0x1530000
        LDR      R1,[R0, #+4]
        BIC      R1,R1,#0x1E
        STR      R1,[R0, #+4]
...
//   92         CACHE->CACHE_END_ENTRY_N[region] = 0;
        LDR.W    R2,??DataTable14_4  ;; 0x1540040
        STR      R1,[R2, R0, LSL #+2]
//   93 
...
??hal_cache_enable_0:
        LDR.N    R0,??DataTable14_1  ;; 0x1530000
        LDR      R2,[R0, #+0]
        ORR      R2,R2,#0xD
        STR      R2,[R0, #+0]
//  146 
...
//  181         CACHE->CACHE_REGION_EN |= (1 << region);
        LDR.N    R1,??DataTable14_6  ;; 0x153002c
        LDR      R2,[R1, #+0]
        MOVS     R3,#+1
        LSL      R0,R3,R0
        ORRS     R0,R0,R2
...
??hal_cache_region_disable_0:
        LDR.N    R2,??DataTable14_6  ;; 0x153002c
        LDR      R3,[R2, #+0]
        MOVS     R4,#+1
        LSL      R0,R4,R0
        BIC      R0,R3,R0
...
??hal_cache_set_size_0:
        LDR.N    R2,??DataTable14_1  ;; 0x1530000
        LDR      R3,[R2, #+0]
        BIC      R3,R3,#0x300
        STR      R3,[R2, #+0]
...
//  260     CACHE->CACHE_END_ENTRY_N[region] = region_config->cache_region_address + region_config->cache_region_size;
        LDR.N    R2,??DataTable14_4  ;; 0x1540040
        LDR      R4,[R1, #+0]
        LDR      R1,[R1, #+4]
        ADDS     R4,R1,R4
        STR      R4,[R2, R0, LSL #+2]
...
//  285     CACHE->CACHE_OP = (address & CACHE_OP_TADDR_MASK);
        LDR.N    R1,??DataTable14_7  ;; 0x1530004
        LSRS     R4,R4,#+5
        LSLS     R4,R4,#+5
        STR      R4,[R1, #+0]
...
//  406 
        ...
        SECTION `.text`:CODE:NOROOT(1)
          ...
          CFI Function hal_cache_is_cacheable
          CFI NoCalls
        THUMB
...
hal_cache_is_cacheable:
        LDR.N    R1,??DataTable14_1  ;; 0x1530000
        LDR      R2,[R1, #+0]
        LSLS     R2,R2,#+28
        BPL.N    ??hal_cache_is_cacheable_0
//  413         return false;
//  414     }
...
//  416     for (region = HAL_CACHE_REGION_0; region < HAL_CACHE_REGION_MAX; region ++) {
        MOVS     R2,#+0
        B.N      ??hal_cache_is_cacheable_1
??hal_cache_is_cacheable_2:
        ADDS     R2,R2,#+1
        UXTB     R2,R2
??hal_cache_is_cacheable_1:
        CMP      R2,#+16
        BGE.N    ??hal_cache_is_cacheable_0
//  417         /* Only compare with region that is enabled */
//  418         if (CACHE->CACHE_REGION_EN & (1 << region)) {
        LDR      R3,[R1, #+44]
        LSRS     R3,R3,R2
        LSLS     R3,R3,#+31
        BPL.N    ??hal_cache_is_cacheable_2
//  419             if ((address >= (CACHE->CACHE_ENTRY_N[region] & ~CACHE_ENTRY_N_C_MASK)) && (address < CACHE->CACHE_END_ENTRY_N[region])) {
        MOV      R3,#+22282240
        LDR      R3,[R3, R2, LSL #+2]
        BIC      R3,R3,#0x100
        CMP      R0,R3
        BCC.N    ??hal_cache_is_cacheable_2
        LDR.N    R3,??DataTable14_4  ;; 0x1540040
        LDR      R3,[R3, R2, LSL #+2]
        CMP      R0,R3
        BCS.N    ??hal_cache_is_cacheable_2
//  420                 return true;
        MOVS     R0,#+1
        BX       LR
//  421             }
