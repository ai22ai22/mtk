...
//    Endian       =  little
//    Source file  =  
//        C:\LEO\myGit\MTK-IOT\SDK_V4.3.0\driver\board\mt76x7_hdk\util\src\board_cli.c
//    Command line =  
//        -f C:\Users\leo\AppData\Local\Temp\EW4DEF.tmp
//        (C:\LEO\myGit\MTK-IOT\SDK_V4.3.0\driver\board\mt76x7_hdk\util\src\board_cli.c
...
//    Locale       =  C
//    List file    =  
//        C:\LEO\myGit\MTK-IOT\SDK_V4.3.0\out_iar\mt7697_hdk\iot_sdk\Debug\List\board_cli.s
//
///////////////////////////////////////////////////////////////////////////////
...
//   79 #if (PRODUCT_VERSION == 7687 || PRODUCT_VERSION == 7697)
//   80     connsys_util_get_ncp_patch_ver(patch_ver);
        MOV      R0,SP
          CFI FunCall connsys_util_get_ncp_patch_ver
        BL       connsys_util_get_ncp_patch_ver
//   81 #endif
//   82     cli_puts("N9 Image  Ver: ");
        ADR.W    R0,?_2
          CFI FunCall cli_puts
        BL       cli_puts
//   83     cli_puts(fw_ver);
...
//   84     cli_putln();
          CFI FunCall cli_putln
        BL       cli_putln
//   85 #if (PRODUCT_VERSION == 7687 || PRODUCT_VERSION == 7697)
//   86     cli_puts("HW Patch  Ver: ");
        ADR.W    R0,?_3
          CFI FunCall cli_puts
        BL       cli_puts
//   87     cli_puts(patch_ver);
        MOV      R0,SP
          CFI FunCall cli_puts
        BL       cli_puts
//   88     cli_putln();
...
//   93 
        ...
        THUMB
//   94 uint8_t board_cli_reboot(uint8_t len, char *param[])
//   95 {
board_cli_reboot:
        PUSH     {R7,LR}
          CFI R14 Frame(CFA, -4)
          CFI CFA R13+8
//   96     cli_puts("Reboot Bye Bye Bye!!!!\n");
        ADR.W    R0,?_4
          CFI FunCall cli_puts
        BL       cli_puts
//   97 
//   98     hal_cache_disable();
          CFI FunCall hal_cache_disable
        BL       hal_cache_disable
//   99     hal_cache_deinit();
          CFI FunCall hal_cache_deinit
        BL       hal_cache_deinit
//  100 #if (PRODUCT_VERSION == 7687) || (PRODUCT_VERSION == 7697)
//  101     hal_sys_reboot(HAL_SYS_REBOOT_MAGIC, WHOLE_SYSTEM_REBOOT_COMMAND);
        MOVS     R1,#+0
        LDR.N    R0,??DataTable10  ;; 0x20160626
          CFI FunCall hal_sys_reboot
        BL       hal_sys_reboot
//  102 #else
//  103     cli_puts("Reboot not Ready !\n");
//  104 #endif
//  105     return 0;
        MOVS     R0,#+0
        POP      {R1,PC}          ;; return
...
//  108 
        ...
        THUMB
//  109 uint8_t board_cli_reg_read(uint8_t len, char *param[])
//  110 {
board_cli_reg_read:
        PUSH     {R7,LR}
          CFI R14 Frame(CFA, -4)
...
//  124     } else {
//  125         val = *((volatile uint32_t *)reg);
??board_cli_reg_read_1:
        LDR      R3,[R0, #+0]
//  126         printf("read register 0x%08x (%u) got 0x%08x\n", (unsigned int)reg, (unsigned int)reg, (unsigned int)val);
        MOV      R2,R0
        MOV      R1,R0
        ADR.W    R0,?_6
          CFI FunCall printf
...
//  132 
        ...
        THUMB
//  133 uint8_t board_cli_reg_write(uint8_t len, char *param[])
//  134 {
board_cli_reg_write:
        PUSH     {R3-R5,LR}
          CFI R14 Frame(CFA, -4)
...
//  149         }
//  150 
//  151         *((volatile uint32_t *)reg) = val;
??board_cli_reg_write_2:
        STR      R0,[R5, #+0]
//  152         printf("written register 0x%08x (%u) as 0x%08x\n", (unsigned int)reg, (unsigned int)reg, (unsigned int)val);
        MOV      R3,R0
        MOV      R2,R5
        MOV      R1,R5
        ADR.W    R0,?_8
