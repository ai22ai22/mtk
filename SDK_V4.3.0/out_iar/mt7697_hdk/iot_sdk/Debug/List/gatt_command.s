...
//    Locale       =  C
//    List file    =  
//        C:\LEO\myGit\MTK-IOT\SDK_V4.3.0\out_iar\mt7697_hdk\iot_sdk\Debug\List\gatt_command.s
//
///////////////////////////////////////////////////////////////////////////////
...
??find_char_node_by_handle_range_0:
        ADD      R3,R0,#+36
        CMP      R2,R3
        BEQ.N    ??find_char_node_by_handle_range_1
//  216     {
...
//  221             (handle <= service_node->service.end_handle))
        ...
        ADD      R5,R0,#+36
        CMP      R3,R5
        BNE.N    ??find_char_node_by_handle_range_2
        MOV      R5,R1
        MOV      R6,R4
        CMP      R5,R6
        BCC.N    ??find_char_node_by_handle_range_2
        LDRH     R5,[R0, #+10]
        MOV      R6,R1
        CMP      R5,R6
        BCS.N    ??find_char_node_by_handle_range_3
//  222         {
...
??find_char_node_by_handle_range_2:
        MOV      R5,R1
        CMP      R5,R4
        BCC.N    ??find_char_node_by_handle_range_4
        MOV      R4,R1
        LDRH     R5,[R3, #+8]
        CMP      R4,R5
...
//  349     LOG_I(common, "  [GATT]Include Service Handle = 0x%04x", inc_srv->handle);
        LDR.W    R5,??DataTable104_3
        ADR.W    R6,`print_include_service::__FUNCTION__`
        LDRH     R0,[R4, #+20]
        STR      R0,[SP, #+4]
        ADR.W    R0,?_5
        STR      R0,[SP, #+0]
...
??print_all_include_service_0:
        CMP      R4,#+0
        BEQ.N    ??print_all_include_service_1
        CMP      R4,R5
        BEQ.N    ??print_all_include_service_1
...
//  463     if (bt_uuid_is_uuid16(&srv->uuid)) {
        LDR.W    R5,??DataTable104_3
        ADR.W    R6,`print_service::__FUNCTION__`
        ADDS     R0,R4,#+4
          CFI FunCall bt_uuid_is_uuid16
        BL       bt_uuid_is_uuid16
        CMP      R0,#+0
...
??bt_app_cmd_gattc_event_callback_0:
        LDR.W    R5,??DataTable106_2
        ADR.W    R6,`bt_app_cmd_gattc_event_callback::__FUNCTION__`
        STR      R4,[SP, #+4]
        ADR.W    R0,?_30
        STR      R0,[SP, #+0]
        MOVS     R3,#+1
...
??bt_app_cmd_gattc_event_callback_1:
        LDR.W    R6,??DataTable106_2
        ADR.W    R7,`bt_app_cmd_gattc_event_callback::__FUNCTION__`
        STR      R4,[SP, #+4]
        ADR.W    R0,?_32
        STR      R0,[SP, #+0]
        MOVS     R3,#+1
...
//  896             for (i = 0; i < num_of_data; i++){
        MOV      R10,#+0
        STR      R0,[SP, #+36]
        LDR.W    R11,??DataTable105_1
??bt_app_cmd_gattc_event_callback_12:
        LDR      R0,[SP, #+36]
        CMP      R10,R0
        BGE.W    ??bt_app_cmd_gattc_event_callback_13
...
//  901                 if (rsp->att_rsp->length == 6) {
        ...
        MOV      R1,R0
        LDRB     R2,[R0, #+1]
        BEQ.N    ??bt_app_cmd_gattc_event_callback_14
...
//  908                     dump_uuid128(&uuid128);
        ...
        BL       dump_uuid128
        B.N      ??bt_app_cmd_gattc_event_callback_15
//  909                 }
...
??bt_app_cmd_gattc_event_callback_15:
        LDRB     R0,[R11, #+14]
        LSLS     R0,R0,#+31
        BPL.N    ??bt_app_cmd_gattc_event_callback_16
...
//  918                     if (srv_count >= MAX_SERVICE_NO) {
        UXTH     R0,R0
        CMP      R0,#+50
        BLT.N    ??bt_app_cmd_gattc_event_callback_17
...
??bt_app_cmd_gattc_event_callback_17:
        CMP      R5,#+0
        BEQ.N    ??bt_app_cmd_gattc_event_callback_16
...
//  940             if (status != BT_STATUS_SUCCESS && status != BT_ATT_ERRCODE_CONTINUE) {
        CMP      R4,#+0
        BEQ.N    ??bt_app_cmd_gattc_event_callback_18
        CMP      R4,#+129
        BEQ.N    ??bt_app_cmd_gattc_event_callback_18
...
??bt_app_cmd_gattc_event_callback_19:
        CMP      R11,R10
        BGE.W    ??bt_app_cmd_gattc_event_callback_20
...
//  963                 if (rsp->att_rsp->length <= 8) {
        ...
        MUL      R0,R0,R11
        ADD      R1,R1,R0
        BLE.N    ??bt_app_cmd_gattc_event_callback_21
...
??bt_app_cmd_gattc_event_callback_23:
        ADD      R11,R11,#+1
        B.N      ??bt_app_cmd_gattc_event_callback_19
//  985 
...
//  998             if (status != BT_STATUS_SUCCESS && status != BT_ATT_ERRCODE_CONTINUE) {
        CMP      R4,#+0
        BEQ.N    ??bt_app_cmd_gattc_event_callback_24
        CMP      R4,#+129
        BEQ.N    ??bt_app_cmd_gattc_event_callback_24
...
??bt_app_cmd_gattc_event_callback_25:
        CMP      R11,R10
        BGE.W    ??bt_app_cmd_gattc_event_callback_26
...
// 1020                 if (rsp->att_rsp->length < 20) {
        ...
        MUL      R0,R0,R11
        ADD      R1,R1,R0
        BLT.N    ??bt_app_cmd_gattc_event_callback_27
...
??bt_app_cmd_gattc_event_callback_29:
        ADD      R11,R11,#+1
        B.N      ??bt_app_cmd_gattc_event_callback_25
// 1043 
...
// 1055             if (status != BT_STATUS_SUCCESS && status != BT_ATT_ERRCODE_CONTINUE) {
        CMP      R4,#+0
        BEQ.N    ??bt_app_cmd_gattc_event_callback_30
        CMP      R4,#+129
        BEQ.N    ??bt_app_cmd_gattc_event_callback_30
...
// 1070             if (format == 0x02) {
        CMP      R0,#+2
        BNE.N    ??bt_app_cmd_gattc_event_callback_31
// 1071                 attribute_length = 18;
        MOV      R9,#+18
        B.N      ??bt_app_cmd_gattc_event_callback_32
// 1072             }
// 1073             else {
...
??bt_app_cmd_gattc_event_callback_33:
        CMP      R11,R10
        BGE.W    ??bt_app_cmd_gattc_event_callback_34
...
// 1079                 if (format == 0x02) {
        ...
        CMP      R0,#+2
        LDR      R0,[R8, #+4]
        BEQ.N    ??bt_app_cmd_gattc_event_callback_35
// 1080                     /* uuid 128 */
...
// 1121             if (status != BT_STATUS_SUCCESS) {
        CMP      R4,#+0
        BEQ.N    ??bt_app_cmd_gattc_event_callback_38
...
// 1130             for (i = 0; i < length; i++)
        MOVS     R7,#+0
        B.N      ??bt_app_cmd_gattc_event_callback_39
...
??bt_app_cmd_gattc_event_callback_39:
        CMP      R7,R4
        BLT.N    ??bt_app_cmd_gattc_event_callback_40
// 1132             printf("\n");
        ADR.N    R0,??DataTable105  ;; "\n"
          CFI FunCall printf
        BL       printf
...
// 1159             if (status != BT_STATUS_SUCCESS) {
        CMP      R4,#+0
        BEQ.N    ??bt_app_cmd_gattc_event_callback_43
...
??bt_app_cmd_gattc_event_callback_13:
        ...
        LDRB     R0,[R0, #+14]
        LSLS     R0,R0,#+31
        BPL.N    ??bt_app_cmd_gattc_event_callback_9
...
// 1231         ut_app_callback = bt_app_cmd_gattc_event_callback;
        LDR.N    R0,??DataTable106_4
        LDR.N    R1,??DataTable106_5
        STR      R0,[R1, #+0]
// 1232 
...
??bt_cmd_gattc_io_callback_4:
        CMP      R0,#+0
        BMI.N    ??bt_cmd_gattc_io_callback_8
        ADDS     R2,R4,R0
        LDRB     R2,[R2, #+21]
        MOV      R3,R2
        SUB      R6,R3,#+48
        CMP      R6,#+10
        BCS.N    ??bt_cmd_gattc_io_callback_7
        SUBS     R2,R2,#+48
        ADD      R3,SP,#+12
        STRB     R2,[R3, R1]
        B.N      ??bt_cmd_gattc_io_callback_6
...
?_1:
        ...
        DC8 73H, 72H, 63H, 5CH, 75H, 74H, 5FH, 61H
        DC8 70H, 70H, 5CH, 67H, 61H, 74H, 74H, 5FH
        DC8 63H, 6FH, 6DH, 6DH, 61H, 6EH, 64H, 2EH
        DC8 63H, 0
        DC8 0, 0
