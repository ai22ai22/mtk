...
//    Endian       =  little
//    Source file  =  
//        C:\LEO\myGit\MTK-IOT\SDK_V4.3.0\driver\chip\mt7687\src\hal_rtc.c
//    Command line =  
//        -f C:\Users\leo\AppData\Local\Temp\EW7982.tmp
//        (C:\LEO\myGit\MTK-IOT\SDK_V4.3.0\driver\chip\mt7687\src\hal_rtc.c -D
...
//    Locale       =  C
//    List file    =  
//        C:\LEO\myGit\MTK-IOT\SDK_V4.3.0\out_iar\mt7697_hdk\iot_sdk\Debug\List\hal_rtc.s
//
///////////////////////////////////////////////////////////////////////////////
...
//  241     uint32_t irq_status;
//  242     hal_nvic_save_and_set_interrupt_mask(&irq_status);
        MOV      R0,SP
          CFI FunCall hal_nvic_save_and_set_interrupt_mask
        BL       hal_nvic_save_and_set_interrupt_mask
//  243 
//  244     if (HAL_RTC_SLEEP_MAGIC == magic) {
        LDR.N    R0,??DataTable5_6  ;; 0xbabebabe
        CMP      R4,R0
        BNE.N    ??hal_rtc_sleep_0
//  245         ret = halRTCEnterMode();
//  246         if (HAL_RET_FAIL == ret) {
          CFI FunCall halRTCEnterMode
        BL       halRTCEnterMode
        CMP      R0,#+1
        LDR      R0,[SP, #+0]
        BNE.N    ??hal_rtc_sleep_1
//  247             hal_nvic_restore_interrupt_mask(irq_status);
          CFI FunCall hal_nvic_restore_interrupt_mask
        BL       hal_nvic_restore_interrupt_mask
//  248             return HAL_RTC_STATUS_ERROR;
        MVN      R0,#+1
        POP      {R1,R2,R4,PC}
//  249         }
//  250     } else {
//  251         hal_nvic_restore_interrupt_mask(irq_status);
//  252         return HAL_RTC_STATUS_INVALID_PARAM;
//  253     }
//  254     hal_nvic_restore_interrupt_mask(irq_status);
??hal_rtc_sleep_1:
          CFI FunCall hal_nvic_restore_interrupt_mask
        BL       hal_nvic_restore_interrupt_mask
//  255     return HAL_RTC_STATUS_OK;
        MOVS     R0,#+0
        POP      {R1,R2,R4,PC}
??hal_rtc_sleep_0:
        LDR      R0,[SP, #+0]
...
//  257 
        ...
        THUMB
//  258 hal_rtc_status_t hal_rtc_set_data(uint16_t addr, const char *buf, uint16_t len)
//  259 {
hal_rtc_set_data:
        PUSH     {R7,LR}
          CFI R14 Frame(CFA, -4)
          CFI CFA R13+8
//  260     ENUM_HAL_RET_T ret = HAL_RET_FAIL;
//  261 
//  262     ret = halRTCSetBackup(addr, buf, len);
//  263 
//  264     if (HAL_RET_FAIL == ret) {
          CFI FunCall halRTCSetBackup
        BL       halRTCSetBackup
        CMP      R0,#+1
        BNE.N    ??hal_rtc_set_data_0
//  265         return HAL_RTC_STATUS_ERROR;
        MVN      R0,#+1
        POP      {R1,PC}
//  266     }
//  267 
//  268     return HAL_RTC_STATUS_OK;
??hal_rtc_set_data_0:
        MOVS     R0,#+0
        POP      {R1,PC}          ;; return
//  269 }
...
//  270 
        ...
        THUMB
//  271 hal_rtc_status_t hal_rtc_get_data(uint16_t addr, char *buf, uint16_t len)
//  272 {
hal_rtc_get_data:
        PUSH     {R7,LR}
          CFI R14 Frame(CFA, -4)
          CFI CFA R13+8
//  273     ENUM_HAL_RET_T ret = HAL_RET_FAIL;
//  274 
//  275     ret = halRTCGetBackup(addr, buf, len);
//  276 
//  277     if (HAL_RET_FAIL == ret) {
          CFI FunCall halRTCGetBackup
        BL       halRTCGetBackup
        CMP      R0,#+1
        BNE.N    ??hal_rtc_get_data_0
//  278         return HAL_RTC_STATUS_ERROR;
        MVN      R0,#+1
        POP      {R1,PC}
//  279     }
//  280 
//  281     return HAL_RTC_STATUS_OK;
??hal_rtc_get_data_0:
        MOVS     R0,#+0
        POP      {R1,PC}          ;; return
//  282 }
...
//  283 
        ...
        THUMB
//  284 hal_rtc_status_t hal_rtc_clear_data(uint16_t addr, uint16_t len)
//  285 {
hal_rtc_clear_data:
        PUSH     {R7,LR}
          CFI R14 Frame(CFA, -4)
          CFI CFA R13+8
//  286     ENUM_HAL_RET_T ret = HAL_RET_FAIL;
//  287 
//  288     ret = halRTCClearBackup(addr, len);
//  289 
//  290     if (HAL_RET_FAIL == ret) {
          CFI FunCall halRTCClearBackup
        BL       halRTCClearBackup
        CMP      R0,#+1
        BNE.N    ??hal_rtc_clear_data_0
//  291         return HAL_RTC_STATUS_ERROR;
        MVN      R0,#+1
        POP      {R1,PC}
//  292     }
//  293 
//  294     return HAL_RTC_STATUS_OK;
...
//  296 
        ...
        THUMB
//  297 hal_rtc_status_t hal_rtc_get_f32k_frequency(uint32_t *frequency)
//  298 {
hal_rtc_get_f32k_frequency:
        PUSH     {R3-R5,LR}
          ...
          CFI CFA R13+16
        MOV      R5,R0
//  299     uint32_t frequency_cr = 0;
        MOVS     R4,#+0
//  300     uint32_t measure_result = 0;
//  301     uint32_t cr_data = 0;
//  302     uint32_t data = 0;
        MOV      R0,R4
        STR      R0,[SP, #+0]
//  303 
//  304     /* check 32k source with strap pin GPIO6*/
//  305     hal_gpio_init(HAL_GPIO_6);
        MOVS     R0,#+6
          CFI FunCall hal_gpio_init
        BL       hal_gpio_init
//  306     hal_gpio_set_direction(HAL_GPIO_6, HAL_GPIO_DIRECTION_INPUT);
        MOV      R1,R4
        MOVS     R0,#+6
          CFI FunCall hal_gpio_set_direction
        BL       hal_gpio_set_direction
//  307     hal_gpio_get_input(HAL_GPIO_6, (hal_gpio_data_t *)&data);
        MOV      R1,SP
        MOVS     R0,#+6
          CFI FunCall hal_gpio_get_input
        BL       hal_gpio_get_input
//  308 
//  309     cr_data = *(volatile uint32_t *)0x81021100;
        LDR.N    R1,??DataTable5_7  ;; 0x81021100
        LDR      R2,[R1, #+0]
//  310     cr_data &= 0xfffcffff; // clear bit 17:16
        BIC      R2,R2,#0x30000
//  311     if (data == 0) {
        LDR      R0,[SP, #+0]
        CMP      R0,#+0
        BNE.N    ??hal_rtc_get_f32k_frequency_0
//  312         //use internal 32k
//  313         cr_data |= 0x20000; // bit[17:16] = 2
        ORR      R2,R2,#0x20000
...
//  315         //use external 32k
//  316         cr_data |= 0x10000; // bit[17:16] = 1
        ORR      R2,R2,#0x10000
//  317     }
//  318     *(volatile uint32_t *)0x81021100 = cr_data;
??hal_rtc_get_f32k_frequency_1:
        STR      R2,[R1, #+0]
//  319 
//  320     /* CNT_CYCLE[2:0]=6, OSC_CK */
//  321     *FRE_DET_CR = 0x000e0000;
...
//  322 
//  323     while (!(frequency_cr & 0x00008000)) {
//  324         frequency_cr = *FRE_DET_CR;
...
//  326 
//  327     measure_result = *FRE_DET_CR & 0x1fff;
        LDR      R0,[R1, #+0]
        UBFX     R0,R0,#+0,#+13
//  328 
//  329     *frequency = 40000000 * 6 / measure_result;
        LDR.N    R1,??DataTable5_9  ;; 0xe4e1c00
        UDIV     R0,R1,R0
        STR      R0,[R5, #+0]
//  330 
//  331     return HAL_RTC_STATUS_OK;
        MOVS     R0,#+0
...
//  333 
//  334 
//  335 #endif /* HAL_RTC_MODULE_ENABLED */
//  336 
// 
