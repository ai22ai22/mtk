...
//    Endian       =  little
//    Source file  =  
//        C:\LEO\myGit\MTK-IOT\SDK_V4.3.0\driver\chip\mt7687\src\common\spi_flash.c
//    Command line =  
//        -f C:\Users\leo\AppData\Local\Temp\EWADC3.tmp
//        (C:\LEO\myGit\MTK-IOT\SDK_V4.3.0\driver\chip\mt7687\src\common\spi_flash.c
...
//    Locale       =  C
//    List file    =  
//        C:\LEO\myGit\MTK-IOT\SDK_V4.3.0\out_iar\mt7697_hdk\iot_sdk\Debug\List\spi_flash.s
//
///////////////////////////////////////////////////////////////////////////////
...
//   78 };
//   79 ATTR_RODATA_IN_TCM unsigned char cmdlist_suspend_winbond[] = {
//   80     SPI, 1, 0x75, SF_UNDEF
//   81 };
//   82 ATTR_RODATA_IN_TCM unsigned char cmdlist_resume_winbond[] = {
...
//  187 {
//  188     sfc_Command_List((const unsigned char *)cmdlist_suspend_winbond);
...
//  196 {
//  197     sfc_Command_List((const unsigned char *)cmdlist_resume_winbond);
...
//  286     gucFlashSFCMode = sfc_GetDirectMode();
        ...
        LDR.W    R1,??DataTable64_3
        STR      R0,[R1, #+0]
//  287 
//  288     if (support_flash_id == SF_TYPE_CUSTOMER) {
        LDR.W    R4,??DataTable64_2
        LDRH     R0,[R4, #+0]
        CMP      R0,#+255
...
//  297                 if(0x40160000 == info->jedec_id || 0x42160000 == info->jedec_id) {
        LDR      R0,[R0, #+8]
        LDR.W    R1,??DataTable64_4  ;; 0x40160000
        CMP      R0,R1
        BEQ.N    ??flash_check_device_5
        LDR.W    R1,??DataTable64_5  ;; 0x42160000
        CMP      R0,R1
...
//  352         sfc_direct_read_setup(DIRECT_READ_SPI_WINBOND, mode);
        MOVS     R1,#+1
        LDR.W    R0,??DataTable64_7  ;; 0xb0b7000
...
??flash_switch_mode_1:
        ...
        MOVS     R1,#+2
        LDR.W    R0,??DataTable64_8  ;; 0xebff9000
          CFI FunCall sfc_direct_read_setup
        BL       sfc_direct_read_setup
        B.N      ??flash_switch_mode_2
??flash_switch_mode_4:
        MOVS     R1,#+2
        LDR.W    R0,??DataTable64_9  ;; 0xebff5000
          CFI FunCall sfc_direct_read_setup
        BL       sfc_direct_read_setup
        B.N      ??flash_switch_mode_2
??flash_switch_mode_3:
        ...
        MOVS     R1,#+3
        LDR.W    R0,??DataTable64_10  ;; 0xbff1000
          CFI FunCall sfc_direct_read_setup
        BL       sfc_direct_read_setup
        B.N      ??flash_switch_mode_2
??flash_switch_mode_5:
...
//  396     log_hal_info("spi device id: %x %x %x (%x)\n", buf[0], buf[1], buf[2], (unsigned int)jedec);
        ADR.W    R7,`flash_config::__FUNCTION__`
        STR      R4,[SP, #+8]
        STR      R0,[SP, #+4]
        STR      R1,[SP, #+0]
        LDRB     R3,[SP, #+12]
...
//  471     log_hal_info("spi device id: %x %x %x (%x)\n", buf[0], buf[1], buf[2], (unsigned int)jedec);
        ADR.W    R8,`flash_init::__FUNCTION__`
        STR      R4,[SP, #+8]
        STR      R1,[SP, #+4]
        STR      R0,[SP, #+0]
        LDRB     R3,[SP, #+12]
...
??flash_fast_read_3:
        CMP      R0,R6
        BGE.N    ??flash_fast_read_4
        CMP      R0,#+28
        BCS.N    ??flash_fast_read_4
//  605         cmd[4 + i] = 0x0;
        MOV      R3,SP
        ADD      R3,R3,R0
        STRB     R2,[R3, #+4]
...
??query_spi_flash_status_2:
        CMP      R1,#+3
        BEQ.N    ??query_spi_flash_status_3
        CMP      R1,#+4
        BEQ.N    ??query_spi_flash_status_3
        CMP      R1,#+5
        BNE.N    ??query_spi_flash_status_4
...
// 1096 }
        ...
        DATA
??DataTable64:
        DC32     cmdlist_suspend_winbond

        SECTION `.ramTEXT`:CODE:NOROOT(2)
        SECTION_TYPE SHT_PROGBITS, 0
        DATA
??DataTable64_1:
        DC32     cmdlist_resume_winbond
...
// 3 878 bytes in section .ramTEXT
//   192 bytes in section .rodata
// 
// 3 878 bytes of CODE  memory
