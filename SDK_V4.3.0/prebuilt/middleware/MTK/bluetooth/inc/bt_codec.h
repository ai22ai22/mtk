...
typedef enum {
    BT_CODEC_MEDIA_HFP_MIX_STREAM_OUT_METHOD_NO_MIX = 0,      /**< Maintain origin stream out*/
    BT_CODEC_MEDIA_HFP_MIX_STREAM_OUT_METHOD_REPLACE,         /**< Replace origin stream out*/
...
/** @brief bt codec hfp device path enum */
typedef enum {
    BT_CODEC_HFP_DEVICE_PATH_INTERNAL = 0,                /**< The stream out and stream in path of HFP is to/from internal ABB. */
    BT_CODEC_HFP_DEVICE_PATH_MASTER_I2S,                  /**< The stream out and stream in path of HFP is from MASTER I2S.Note stream out also can to internal ABB. */
    BT_CODEC_HFP_DEVICE_PATH_SLAVE_I2S,                   /**< Not support now. The stream out and stream in path of HFP is from SLAVE I2S.*/
...
/**
 * @brief     Mix a tone to the stream out tone.
...
/**
 * @brief      Query the sample_count of mix stream out buffer.
 * @param[out] sample_count is the size of the mix stream out buffer (in bytes).
 * @return     #BT_CODEC_MEDIA_STATUS_MEDIA_OK, if OK.
 * @note       Call this function before #bt_codec_hfp_mix_stream_out() to known the maximum mix stream stream out buffer.
...
/**
 * @brief     Unregister the callback function for mix stream out data
 * @return    #BT_CODEC_MEDIA_STATUS_MEDIA_OK, if OK.
 */
...
/**
 * @brief     Register the callback function for mixing stream out data
...
/**
 * @brief     Unregister the callback function for getting stream in data
 * @return    #BT_CODEC_MEDIA_STATUS_MEDIA_OK, if OK.
 */
bt_codec_media_status_t bt_codec_hfp_unregister_get_stream_in_data_callback(void);

/**
 * @brief     Register the callback function for get stream in data
 * @param[in] callback is the function pointer of callback for getting stream int data.
 * @param[in] user_data is extended parameter for user.
 * @return    #BT_CODEC_MEDIA_STATUS_MEDIA_OK, if OK.
 */
...
/**
 * @brief     This function sets the SW AAC DECODER for BT A2DP.
 * @param[in] flag is used to determine to turn on or turn off SW AAC DECODER mechanism. true: Decode AAC by CM4, false: Decode AAC by DSP.
 * @return    If the operation completed successfully, the return value is #BT_CODEC_MEDIA_STATUS_OK, otherwise the return value is #BT_CODEC_MEDIA_STATUS_ERROR.
 */
bt_codec_media_status_t bt_codec_a2dp_set_sw_aac_flag(bool flag);

/**
 * @brief     This function gets the SW AAC DECODER for BT A2DP.
 * @return    If the SW AAC DECODER is enabled, the return value is #true, otherwise the return value is #false.
 */
