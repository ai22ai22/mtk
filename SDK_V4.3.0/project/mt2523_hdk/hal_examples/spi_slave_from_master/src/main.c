    ...
    if (HAL_SPI_SLAVE_FSM_SUCCESS_OPERATION == (status.fsm_status)) {
        ...
        switch (slave_status) {
            ...
            case HAL_SPI_SLAVE_EVENT_CRD_FINISH:
                ...
                hal_spi_slave_get_master_read_config(SPI_TEST_SLAVER, &request_address, &request_length);
                hal_spi_slave_send(SPI_TEST_SLAVER, wr_buffer, request_length);
            ...
            case HAL_SPI_SLAVE_EVENT_CWR_FINISH:
                ...
                hal_spi_slave_get_master_write_config(SPI_TEST_SLAVER, &request_address, &request_length);
                hal_spi_slave_receive(SPI_TEST_SLAVER, wr_buffer, request_length);
...
static void spi_slave_receive_data_two_boards_example(void)
{
    ...
    if (HAL_SPI_SLAVE_STATUS_OK != hal_spi_slave_init(SPI_TEST_SLAVER, &spi_configure)) {
    ...
    while (spi_slave_poweron_flag == false) {
        if (spi_slave_timeout_flag == true) {
            printf("Slave detect timeout error\r\n");
            return;
        }
    }
    ...
    /* Step7: check whether the data value is correct. */
    printf("Start to check data integrity\r\n");
    for (i = 0; i < SPI_TEST_DATA_SIZE; i++) {
        if (wr_buffer[i] != ((i + 1) % SPI_TEST_DATA_RANGE)) {
    ...
    printf("Slave is waiting for cfg_read command\r\n");
    ...
    printf("Slave received cfg_read command\r\n\r\n");

    /* Step9: wait SPI master to send read data command. */
    printf("Slave is waiting for read data command\r\n");
    ...
    printf("Slave received read data command\r\n\r\n");
    ...
    /* Step11: De-initialize GPIO after data transaction end. */
