...
ble_dogp_adp_buffer_t g_dogp_adp_buffer = {0};

static uint16_t g_read_len = 0;
static char *notify_string = "ios indication";
...
uint32_t ble_dogp_adp_read_evt_handler(uint16_t conn_handle, void *read_buffer, uint16_t read_length, ble_dogp_service_evt_t *p_evt);
uint32_t ble_dogp_adp_write_evt_handler(uint16_t conn_handle, ble_dogp_service_evt_t *p_evt);
