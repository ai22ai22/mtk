...
static void wifi_host_main_task(void *param)
{
...
#if 0  //WIFI_CONFIG_BY_HOST    //This is under developping,  in present, all wifi iniitial setting are set in MT5932 project
    wifi_host_system_config_t wifi_host_sys_cfg;   
    uint8_t sta_mac[WIFI_MAC_ADDRESS_LENGTH] = {0x00,0x0c,0x59,0x32,0x01,0x0};
    ...
    strcpy((char *)wifi_host_sys_cfg.wifi_config.sta_config.ssid, (const char *)"BodhiNH");
    ...
    /*Get the mac address from 5932 device*/
    uint8_t sta_mac_address[WIFI_MAC_ADDRESS_LENGTH] = {0};

    wifi_host_config_get_mac_address(WIFI_PORT_STA, sta_mac_address);
    LOG_I(common, "STA MAC: 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,\n\r",
        sta_mac_address[0],sta_mac_address[1],sta_mac_address[2],
        sta_mac_address[3],sta_mac_address[4],sta_mac_address[5]);
