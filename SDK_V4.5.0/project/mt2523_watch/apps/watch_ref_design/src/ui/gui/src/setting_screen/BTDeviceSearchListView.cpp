...
void BTDeviceSearchListView::listElementClicked(MenuListElement &element)
{
...
#else
    if (bt_connection_app_get_state() == BT_CONNECTION_APP_STATE_SEARCHING) {
    ...
    bt_connection_app_set_state(BT_CONNECTION_APP_STATE_PAIRING);
     bt_connection_app_connect(true);
#endif
...
{
#ifndef SIMULATOR
    if ((bt_connection_app_have_update()) == true) { //search list update.
        refreshSearchList();
        bt_connection_app_set_update_flag(false);
    }
#endif
}
...
void BTDeviceSearchListView::refreshSearchList()
{
    ...
    for (uint32_t i = 0; i < list_count; i++) {
        ...
        if (array_size > 11) {
            Unicode::strncpy(truncate, search_dev->dev_name, 11);
