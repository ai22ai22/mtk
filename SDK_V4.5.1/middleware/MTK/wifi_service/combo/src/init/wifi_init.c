...
static void wifi_build_config_from_sys_cfg(wifi_config_t *config, sys_cfg_t *syscfg )
{
...
static void wifi_build_config_ext_from_sys_cfg(wifi_config_ext_t *config_ext, sys_cfg_t *syscfg)
{
...
void wifi_sys_cfg_init(sys_cfg_t *config)
{
     wifi_config_t *user_config = NULL;
     wifi_config_ext_t *user_config_ext = NULL;
#ifdef MTK_WIFI_PROFILE_ENABLE
    wifi_get_config_from_nvdm(config);
#else
    wifi_get_default_config(config);
#endif

#ifdef MTK_CM4_WIFI_TASK_ENABLE
    fw_get_user_config(&user_config,&user_config_ext);
#endif
    wifi_apply_user_config(config, user_config, user_config_ext);
}
...
void wifi_init(wifi_config_t *config, wifi_config_ext_t *config_ext)
{
    ...
    wifi_config_ext_t default_config_ext = {0};
    ...
    if (NULL == pconfig) {
        ...
        wifi_build_config_from_sys_cfg(&default_config, &syscfg);
        pconfig = &default_config;
    }
    if (NULL == pconfig_ext) {
        LOG_I(wifi, "config_ext is null. Will use default config_ext configuration");
        wifi_build_config_ext_from_sys_cfg(&default_config_ext, &syscfg);
...
#endif
#ifdef MTK_CM4_WIFI_TASK_ENABLE
    wifi_firmware_init(pconfig, pconfig_ext);
