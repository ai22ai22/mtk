...
static void smtcn_rcv_pkt_thread(void *pvParameters)
{
    ...
    if((SMTCN_FLAG_EXIT == smtcn_flag) || (SMTCN_FLAG_FAIL == smtcn_flag)) {
        ...
        wifi_event_notification(WIFI_PORT_STA, EVENT_ID_IOT_SMT_STOP, NULL, 0);
#endif
...
#ifdef MTK_WIFI_ROM_ENABLE
        wifi_event_notification(WIFI_PORT_STA, EVENT_ID_IOT_SMT_START, NULL, 0);
        /*just left a small period for processing linkdown event*/
        vTaskDelay(100);
#endif 
