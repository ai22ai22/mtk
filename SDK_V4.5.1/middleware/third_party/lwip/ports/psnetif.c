...
low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  sprintf(data, "AT+EIPDATA=%d,%d,\"", (int)netif->num, (int)total_len);
  if (total_len == p->len) {
...
#ifdef __PSNETIF_SUPPORT_CMUX_DATA_CHANNEL__
  channel_id = cmux_get_channel_id(netif->num);
  LWIP_DEBUGF(NETIF_DEBUG, ("low_level_output:netif->num = %d, channel_id = %d\n", netif->num, channel_id));
